...
每个 lineage lock record 按 `{tenantId, sourceScopeReference, mappingReference, candidateDigest, candidatePairDigest, evidenceFingerprint}` 绑定到具体 mapping aggregate 与候选 lineage，一经创建即不可修改或解除；conflict record 还必须不可变地绑定唯一 `conflictOrigin` / `conflictType` pair。跨 aggregate 的稳定对照键固定为 `{candidatePairDigest, evidenceFingerprint}`。`candidatePairDigest` 独立绑定 tenant、稳定 source scope、external contact digest 与 system customer digest，不包含 aggregate / candidate version、mapping reference、timestamp 或完整 source snapshot digest。`conflict` 会创建 conflict 类型 lineage lock；任何 `clear_candidate` 都必须创建或保留 lineage lock，因此非冲突 clear 也不能复活旧 candidate。上述 digest / reference 只能由 domain 从已验证 canonical mock/demo 数据生成；conflict origin / type 只能由下文三个封闭的 domain 创建路径硬编码生成。generation / review 调用方不得提供或覆盖。review 调用方只可把既有 `candidateDigest` 作为 opaque 目标引用提交，application 边界必须严格解析并与当前动作的可信 target candidate 做完全相等比对；该引用不能创建或覆盖 digest、`conflictOrigin` 或 `conflictType`，也不能证明 lineage。target 是否必须 active 或允许引用 historical / locked lineage，由第 3.6 节按动作决定。当前设计不提供 `resolve_conflict` 或 unlock 动作。
...
可信 **LineageLockRecord** 是以 `lockType` 判别的封闭 union，不允许用 nullable `conflictOrigin` / `conflictType` 混合两种来源：

- **ConflictLineageLockRecord** 的 exact allowed keys 固定为：`tenantId`、`sourceScopeReference`、`mappingReference`、`candidateDigest`、`externalContactDigest`、`systemCustomerDigest`、`candidatePairDigest`、`evidenceFingerprint`、`sourceSnapshotDigest`、`lockType`、`conflictOrigin`、`conflictType`、`unresolvedConflictCount`、`createdAt`、`sourceKind`、`dataMode`；`lockType` 必须等于 `conflict`，`conflictOrigin` / `conflictType` 必须命中下述唯一 pair，count 必须为 1—100。
- **ClearanceLineageLockRecord** 的 exact allowed keys 固定为：`tenantId`、`sourceScopeReference`、`mappingReference`、`candidateDigest`、`externalContactDigest`、`systemCustomerDigest`、`candidatePairDigest`、`evidenceFingerprint`、`sourceSnapshotDigest`、`lockType`、`unresolvedConflictCount`、`createdAt`、`sourceKind`、`dataMode`；`lockType` 必须等于 `clearance`，不得出现 `conflictOrigin` 或 `conflictType`，count 必须固定为 0。它只服务无 unresolved conflict 的普通 candidate clear；conflict-origin clear 保留 count 为 1—100 的原 ConflictLineageLockRecord，不创建该 variant。

`conflictOrigin` → `conflictType` 只允许以下三个不对外暴露 origin / type 参数的封闭创建路径：

1. generation 内部已分类为 `multiple_system_customers` 的唯一分支，只能调用硬编码 `generation_multiple_system_customers → multiple_system_customers_for_external_contact` 的 constructor；
2. generation 内部已分类为 `multiple_external_contacts` 的唯一分支，只能调用硬编码 `generation_multiple_external_contacts → multiple_external_contacts_for_system_customer` 的 constructor；
3. 已通过第 3.6 节全部 guard 的 `mark_conflict` 只能调用硬编码 `manual_review_mark_conflict → manual_marked` 的 constructor。

不存在接收任意 `conflictOrigin` 或 `conflictType` 的通用 constructor。generation 在同一已确认 conflict group 同时命中前两类时，只用第 1 项，避免集合遍历顺序导致 type 漂移；这个优先级只分类已经由 generation 确认的冲突，不在本次修订中新建、改写或简化 candidate eligibility / score 契约。`conflictOrigin` 与 `conflictType` 都是封闭枚举且只能命中上述三个 pair；单独格式合法但 pair 不一致同样非法。当前 05C-E1 没有独立 evidence-observation 契约，因此不允许发射 `generation_evidence_inconsistent` 或 `evidence_inconsistent`；source / fixture evidence 无法验证固定按现有 `untrusted_fixture_provenance` 或 `trusted_target_integrity_invalid` 阻断，不伪装成新 conflict origin。

每个 LineageLockRecord 都是 module-owned、write-once 不可变可信状态；LineageLockIndex 则是 append-only 的 versioned snapshot，两者都由 application boundary 注入且不是 raw command。domain 创建新 lock 时只能从上一个完整 index 逐字节保留全部旧 records、追加一个新 record、令 `indexVersion` 严格加 1，再按固定顺序重排并重算 `indexDigest`；不提供修改、替换或删除既有 record 的路径。conflict-origin clear 复用原 record，不追加新 record，因此 indexVersion / indexDigest 不变；普通 clear 只追加一个 count=0 的 ClearanceLineageLockRecord，按上述规则升版。若当前已有 1000 条 records 或 `indexVersion=2,147,483,647`，任何需要追加的 generation conflict、`mark_conflict` 或普通 clear 都必须保持 aggregate / target / index 完全不变，固定按 `mapping_locked_candidate_reuse_blocked / lineage_index_invalid` 阻断；禁止删旧 record 腾位、version wrap、跳版或部分提交。本契约通过封闭 constructor、record exact shape 与 index canonical hash 防止所有已授权 domain 路径中的 origin / type 漂移，但不宣称能从被攻击者整体替换并重算所有 hash 的“可信” application state 中自行恢复历史真值；防止这类越过 domain 的状态权威篡改属于 application/state boundary，不在本次 docs-only 修订中新增 provenance registry、repository 或事务。

所有 digest / reference / timestamp 分别通过第 4.5、5、6 节规则。两种 lock record 的 `candidatePairDigest` 都必须由同一 record 的 tenant、source scope 与两个 source digest 重算，不能由 candidateDigest 或 sourceSnapshotDigest 代替。ConflictLineageLockRecord 的 `conflictOrigin` 与 `conflictType` 都参与 record 的 canonical encoding 和 LineageLockIndex `indexDigest`；任一缺失、多出、pair 不一致或与当前 index hash 不一致，均固定按第 7.2 节 `mapping_locked_candidate_reuse_blocked / lineage_index_invalid` 阻断。
...
- `mappingConflict` 只投影 conflict-origin lineage，是只读摘要而不是解锁命令；普通 candidate 的 `clear_candidate` 会创建 ClearanceLineageLockRecord，但不产生 `mappingConflict`。`conflictType` 只允许 `multiple_system_customers_for_external_contact`、`multiple_external_contacts_for_system_customer`、`manual_marked`，并且必须与原始 ConflictLineageLockRecord 的不可变值完全相等；`conflictStatus` 只允许 `unresolved_locked` / `cleared_locked`。前者必须对应 aggregate `conflict / mapping_conflict`、唯一 conflict record、locked active target、`unresolvedConflictCount=1—100` 与 `manualReviewRequired=true`；后者只表示该 conflict-origin lineage 已执行 clear，必须对应 `cleared_locked / candidate_cleared_locked`、仍保留的同一原始 conflict record、cleared inactive locked target、保留原 conflict count 与 `manualReviewRequired=false`。conflict-origin clear 不创建第二条 clearance record，避免重复 `{candidatePairDigest, evidenceFingerprint}` 对照键。
- `mappingConflict` 的三个 digest 和 `conflictType` 必须逐项绑定同一可信 target 与原始 ConflictLineageLockRecord，`createdAt` 始终等于该 conflict record 的 canonical timestamp；`cleared_locked` 由 aggregate、target 与成功 audit / history 证明 clear 已发生，且必须复用原 type，不改变或替代原 conflict record。type 缺失、错配或翻转固定按 `mapping_input_blocked / derived_output_contract_invalid` 阻断。多个冲突使用多个单-lineage 投影，不得用可变数组合并身份，也不得借该投影删除、替换或解除永久 lock。
...
| 可信 context / aggregate / LineageLockIndex / lock record / target 或 domain output 任一 string value 命中统一 sensitive scanner | `forbidden_field_blocked` | `sensitive_value_blocked` | aggregate 尚未验证时 `not_evaluated → not_evaluated`；验证后保持可信当前状态；generation 为 `unmatched → unmatched` |
...
| canonical hash vectors | 为 `LP` / `CE`、externalContactsDigest、systemCustomersDigest、sourceSnapshotDigest、fixtureRegistryDigest、mappingReference、candidatePairDigest、evidenceFingerprint、candidateDigest、两种 LineageLockRecord 与 LineageLockIndex `indexDigest` 固定至少一组跨实现 known-answer vector；包含多字节 UTF-8 byte length、null / boolean / integer / array/object type tag、ConflictLineageLockRecord 的 `conflictOrigin` / `conflictType` 与每个 domain separator |
...
| generation conflict origin / type | 对 generation 自身已确认的两类 conflict group，分别断言只能通过封闭 constructor 生成 `generation_multiple_system_customers → multiple_system_customers_for_external_contact`、`generation_multiple_external_contacts → multiple_external_contacts_for_system_customer`；同一已确认 group 同时命中两类时固定采用前者，不受集合遍历顺序影响。用例只验证 type 分类，不在本次修订重定义 candidate eligibility / score；generation 不得生成 `manual_review_mark_conflict → manual_marked`，source / fixture evidence 无法验证时不得发射任何 origin / type |
...
| domain output exact shape | 对 `mappingReview`、`mappingDecision`、`mappingConflict` 分别逐字段测试缺键、多键、unknown key、nested raw container、wrong type、grammar / enum / cross-field 不一致，固定断言 `mapping_input_blocked / derived_output_contract_invalid`；`mappingConflict.conflictType` 还要逐一测试缺失、非法 enum、与 origin record 错配、投影间翻转。再对每个 string field（包括 digest、timestamp、enum）逐项注入 phone / secret / externalUserId 模式，固定断言 `forbidden_field_blocked / sensitive_value_blocked`。两类均要求统一 scanner 先于 grammar、factory 不返回部分对象且 mutation 不提交 |
| `mappingConflict` 来源边界 | `manual_review_mark_conflict → manual_marked` 与两类 generation origin / type pair 逐项绑定原始 ConflictLineageLockRecord；任一 pair 错配、人工/生成来源互换或提交 `generation_evidence_inconsistent` / `evidence_inconsistent` 都阻断。conflict-origin lineage 分别覆盖 `unresolved_locked` 与 clear 后 `cleared_locked` 对同一原始 ConflictLineageLockRecord、同一 origin / type pair、aggregate / target 状态与 audit / history 的绑定，并断言 clear 后 pair 不变且不新增重复 clearance record；普通 candidate clearance 明确断言只产生 count=0 的 ClearanceLineageLockRecord、不产生 `mappingConflict`，不得注入或伪造 origin / type |
...
| lineage index / record union | LineageLockIndex / record 非 plain object、逐字段缺失/额外、wrong type；显式覆盖 `lockType=conflict` + Clearance exact keys 和 `lockType=clearance` + Conflict exact keys 两种 literal / key-set 互换；ConflictLineageLockRecord 缺少/非法/错配/翻转 `conflictOrigin` / `conflictType` pair，ClearanceLineageLockRecord 注入 origin / type，以及 pair 改变但复用旧 `indexDigest`；indexVersion/count 边界；`complete=false`；records 缺项、多项、重复、乱序、digest 重算不等；tenant/source scope/mapping/source mode 或 aggregate index digest 不一致均固定 `mapping_locked_candidate_reuse_blocked / lineage_index_invalid`。不构造“攻击者替换全部 module-owned 状态并重算所有 hash”的伪 domain 能力断言 |
| conflict constructor boundary | 三个封闭 constructor 逐一断言只能产生其硬编码 origin / type pair；没有接收任意 origin / type 的通用入口；raw command、trusted target、aggregate 或 projection 中尝试提供/覆盖 pair 均阻断；clear 只复用原 record，不重跑 constructor |
| append-only index 演进 | 追加新 lock 后所有旧 records 逐字节深等、只多 1 项、`indexVersion` 严格加 1 且 `indexDigest` 改变；删除/替换/改写旧 record、跳版或一次多项均阻断。conflict-origin clear 断言不追加、version/digest 不变；普通 clear 断言只追加 count=0 ClearanceLineageLockRecord 并按规则升版。容量边界覆盖 records `999→1000` 接受、`1000→1001` 阻断，version `2,147,483,646→2,147,483,647` 接受、再加 1 阻断；阻断时固定 `mapping_locked_candidate_reuse_blocked / lineage_index_invalid`，且 aggregate / target / index 深等、无删旧 record、无 wrap、无部分提交 |
| trusted record string scan | 分别对 ConflictLineageLockRecord 的 `conflictOrigin` 与 `conflictType` 注入 phone / secret / externalUserId 模式；必须在 enum / pair / index 校验前固定返回 `forbidden_field_blocked / sensitive_value_blocked`，audit 不回显原值 |
