...
- 治疗记录结构化摘要 v1、治疗摘要人工录入 v1、治疗摘要管理 v1、治疗后护理 / 随访联动 v1、随访任务来源治理增强 v1 和标准治疗事件 domain-only 契约已完成；真实 HIS 接入、Webhook、文件导入、外部系统同步、完整治疗记录正文、治疗摘要编辑 / 作废、客服会话、知识库、AI、企业微信、开放平台凭证和计费仍未进入真实实现。
...
- 治疗记录模块：结构化摘要 v1、租户隔离、人工录入 v1、只读管理 v1、治疗后护理 / 随访联动 v1、随访任务来源治理增强 v1 和标准治疗事件 domain-only 契约已完成；后续治疗摘要编辑、作废、客服会话联动和完整治疗记录能力需单独规划，仍不保存完整病历正文。
...
1. Phase 18 Plan Mode：优先评估治疗摘要编辑能力 v1，只允许白名单结构化字段，写审计，不编辑完整治疗记录正文、完整病历正文或咨询全文，不做删除。
...
4. 治疗摘要作废能力 v1：软作废、可追溯、写审计，不硬删除。
5. follow-up 配额 enforcement：单独评估是否将 Phase 15 的人工确认创建接入 `maxFollowUps`。
6. 平台租户状态管理、更多资源配额 enforcement 与完整套餐商业化后台规划。
7. 审计高级治理：导出、告警和复杂风控。
8. 知识库 / RAG 基础版，优先元数据规划，不保存医疗隐私正文。
9. 客服会话和完整治疗记录能力。
10. AI provider、调用日志和 Agent。
11. 企业微信、Webhook、OAuth、API Key。
12. 计费、合同、发票和支付。
...
- 后续建议进入 Phase 18 Plan Mode，优先评估治疗摘要编辑能力 v1；HIS 标准治疗事件 mapper 继续增强和业务事件埋点体系 spec 可作为候选方向，当前不进入 Phase 18 实现。
