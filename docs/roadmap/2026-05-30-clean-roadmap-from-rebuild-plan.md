...
- 机构工作台首页仍主要是静态经营展示，尚未基于现有客户、预约、随访 API 派生运营摘要。
- workspace 与三大业务页的加载态、错误态、空态和占位态需要统一。
- 机构端导航需要明确“已真实接入页面”和“后续占位页面”的边界。
...
- Phase 6 运营页面一致性与 workspace 入口真实化：机构首页接入现有真实 API 摘要、统一页面状态、稳定导航边界和 smoke 测试。
...
1. Phase 6：运营页面一致性与 workspace 入口真实化。
2. 客户详情时间线第一版。
3. 审计日志只读查询基础版。
...
- Phase 6 建议聚焦运营页面一致性与 workspace 入口真实化。
- Phase 6 不新增 API、数据库 schema、migration、权限、认证或租户隔离能力，不进入 AI、知识库、企业微信、开放平台凭证、支付、合同、发票、套餐权益 enforcement、平台租户管理真实功能或治疗记录完整正文。
