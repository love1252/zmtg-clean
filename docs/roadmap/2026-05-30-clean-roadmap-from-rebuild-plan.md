...
- 治疗记录结构化摘要 v1、治疗摘要人工录入 v1、治疗摘要管理 v1、治疗后护理 / 随访联动 v1、随访任务来源治理增强 v1、标准治疗事件 domain-only 契约、治疗摘要编辑能力 v1、治疗摘要作废能力 v1 和治疗项目路径模板 v1 已完成；随访路径运营分析 v1 已完成 docs-only 口径规划但未进入功能实现；真实 HIS 接入、Webhook、文件导入、外部系统同步、完整治疗记录正文、版本历史 / diff 展示、客服会话、知识库、AI、企业微信、开放平台凭证和计费仍未进入真实实现。
...
- 治疗记录模块：结构化摘要 v1、租户隔离、人工录入 v1、只读管理 v1、治疗后护理 / 随访联动 v1、随访任务来源治理增强 v1、标准治疗事件 domain-only 契约、治疗摘要编辑能力 v1、治疗摘要作废能力 v1 和治疗项目路径模板 v1 已完成；随访路径运营分析 v1 已完成 Plan Mode 口径规划；后续版本历史 / diff 展示、客服会话联动、完整治疗记录能力、路径模板 schema / API、随访路径分析实现和外部系统接入需单独规划，仍不保存完整病历正文。
...
5. 随访路径运营分析 v1：Phase 21 Plan Mode 已完成最小口径规划；后续如进入实现，先做基于现有对象的只读派生评估，不做复杂归因模型、图表导出或自动触达。
