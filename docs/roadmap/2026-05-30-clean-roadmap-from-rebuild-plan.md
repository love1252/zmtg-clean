...
- Phase 23 HIS 连接配置状态 service audit reason 映射 Plan Mode 已完成：docs-only 收敛状态 service 实现前置 reason，明确 repository `not_found` 复用 `not_found_or_not_owned`、`invalid_state_transition` 和 `conflict` 复用 `invalid_transition`、`validation_failed` 复用 `invalid_his_connection_payload`，thrown error 不写 denied audit；当前仍未新增状态 service 或 API route，未修改 `src/**`、audit domain、query whitelist、权限、schema 或 migration。
- Phase 23 HIS 连接配置状态 API 闭环已完成，下一步进入凭证管理 Plan Mode：本轮只规划凭证创建、更新、轮换、删除 / 撤销、`credentialRef`、`credentialConfigured`、权限、service、repository / storage、audit、DTO、parser 和测试拆分边界；凭证管理仍未实现，测试连接和真实 HIS adapter 仍需单独规划。
- Phase 23 HIS 连接配置凭证管理总边界 Plan Mode 已完成，下一步进入凭证 repository / storage 边界 Plan Mode：当前只细化未来 repository 安全引用、storage 专用承载、`credentialRef`、`credentialConfigured`、轮换、删除 / 撤销、事务一致性、audit、DTO / read model 和测试拆分；凭证 repository / storage 仍未实现，测试连接和真实 HIS adapter 仍需单独规划。
- Phase 23 HIS 连接配置凭证 repository / storage 最小边界已完成：当前仅实现 repository 层安全 `credentialRef` 写入、清空 / 撤销、轮换为新安全引用、read model / summary 的 `credentialConfigured` 派生，以及 fake in-memory storage 测试抽象；不新增 API、不修改 route / service / parser / 权限 / audit domain / schema / migration，不保存真实凭证，不接真实 secret manager，不做测试连接，不接真实 HIS。
- Phase 23 HIS 连接配置凭证 parser / service / DTO Plan Mode 进入规划：当前只规划凭证 create / update / rotate / clear / revoke payload parser、service 编排、DTO 最小响应、audit 衔接和测试拆分；凭证 API route / permission / audit 仍需后续规划，测试连接和真实 HIS adapter 仍需单独规划。
- Phase 23 HIS 连接配置凭证 parser / service / DTO 最小实现已完成：当前新增凭证 create / update / rotate / clear / revoke parser、service 和 DTO helper，只接受合成 placeholder，不返回 `credentialRef`、idempotencyKey、scoped key 或敏感字段；不新增 API route、不修改权限、不新增 audit action / reason / domain、不改 schema / migration、不处理真实凭证、不做测试连接、不接真实 HIS。下一步是凭证 API route / permission / audit Plan Mode。
- Phase 23 HIS 连接配置凭证 API route / permission / audit Plan Mode 进入规划：当前只规划凭证 create / update / rotate / clear / revoke route、`open_connection:manage_credentials` 权限候选、route denied audit、service allowed audit、DTO / error mapping 和测试拆分；不新增 API route、不修改 `src/**`、不修改权限实现、不新增 audit action / reason / domain / query whitelist、不处理真实凭证、不做测试连接、不接真实 HIS。下一步是凭证 API route / permission / audit 最小实现。
- Phase 23 HIS 连接配置凭证 API route / permission / audit 最小实现已完成：当前新增 `POST /credentials`、`PATCH /credentials`、`POST /credentials/rotate`、`POST /credentials/clear`、`POST /credentials/revoke`，接入 `open_connection:manage_credentials`、route denied audit、service allowed audit、稳定 DTO / error mapping 和权限 / audit / route 测试；仍不新增 schema / migration，不接真实 secret manager，不保存真实凭证，不做测试连接，不接真实 HIS。
...
- Phase 23 HIS 连接配置凭证 audit reason / query whitelist 扩展 Plan Mode 进入规划：当前只规划 provider failure / compensation 相关 audit action、候选 reason、query whitelist、metadata、route / service audit、一致性和测试拆分；provider failure / compensation domain 最小边界已完成，real credential parser / service、测试连接和真实 HIS adapter 仍需单独规划。
- Phase 23 HIS 连接配置凭证 audit reason / query whitelist 最小边界已完成：当前仅补充 provider failure / compensation 相关稳定 audit reason，并复用现有 `reason` query whitelist；继续复用 `open_connection:manage_credentials` action 和 `allowed` / `denied` / `transitioned` result，不新增 audit action / result / metadata schema / audit repository，不修改 route / service / parser / DTO / provider / repository / schema / migration，不实现 provider failure audit 或 compensation audit，不接真实 provider，不处理真实凭证，不做测试连接，不接真实 HIS。
- Phase 23 HIS 连接配置凭证 provider failure audit / service audit Plan Mode 进入规划：当前只规划 provider failure audit 写入职责、service audit fail closed、route audit 去重、compensation audit 后续边界、audit metadata 禁区和测试拆分；audit reason / query whitelist 最小边界已完成，但 compensation audit、real credential parser / service、测试连接和真实 HIS adapter 仍需单独规划。
- Phase 23 HIS 连接配置凭证 provider failure audit / service audit 最小边界已完成：当前 credential service 已对已知 safe provider failure 写 `open_connection` / `manage_credentials` / `denied` audit，复用既有 provider failure reason；provider failure audit 写入失败 fail closed 为 `service_unavailable`，成功后再返回既有稳定 service status；route denied audit 和 success allowed audit 保持不变，不实现 compensation audit、不新增 metadata schema、不改 audit repository、不新增 schema / migration、不接真实 provider、不处理真实凭证、不做测试连接、不接真实 HIS。下一步进入 compensation audit Plan Mode，或 real credential one-time material parser / service Plan Mode。
- Phase 23 HIS 连接配置凭证 compensation audit Plan Mode 进入规划：当前只规划 compensation audit 状态语义、写入职责、metadata / schema 前置边界、outbox / job queue 前置边界、provider failure audit 关联、fail closed / best effort 取舍和后续测试拆分；本阶段仍不实现 compensation audit，不新增 metadata schema / schema / migration / job queue / outbox，不接真实 provider，不处理真实凭证，不做测试连接，不接真实 HIS。
- Phase 23 HIS 连接配置凭证 compensation audit 最小实现前置评估完成，metadata / outbox Plan Mode 进入规划：当前只读盘点确认没有 compensation state 持久化、operationId、audit metadata schema、outbox / job queue 或可追踪 manual review / dead letter 载体，不能在无 schema / 无 outbox 下安全实现 compensation audit；本阶段只规划 metadata 字段、operationId、compensation state、outbox 表、job queue、dead letter / manual review、schema / migration、audit repository、query parser 和测试边界，不新增 schema / migration，不实现 compensation audit，不接真实 provider，不处理真实凭证，不做测试连接，不接真实 HIS。
- Phase 23 HIS 连接配置凭证 compensation metadata / operationId schema 最小边界已完成：当前新增安全 compensation operation 状态承载表、server-generated safe operationId 唯一约束、operation type / compensation state / provider failure category 白名单枚举和 domain-only operation metadata helper；本阶段仅提供 schema / migration / type / test 准备层，不写 compensation audit，不实现 outbox / job queue，不修改 audit repository、route、service、parser、DTO、provider 或 repository，不接真实 provider，不处理真实凭证，不做测试连接，不接真实 HIS。下一步建议进入 outbox / job queue Plan Mode 或 compensation operation repository Plan Mode。
- Phase 23 HIS 连接配置凭证 compensation operation repository Plan Mode 进入规划：当前只规划 compensation operation repository 的最小职责、tenant / connection 隔离、operationId 可信来源、状态流转、retry / stale running / manual review、failure category、DTO / read model、transaction / consistency、outbox / job queue 关系、audit 边界和测试拆分；本阶段 docs-only，不修改 `src/**` 或 `drizzle/**`，不实现 repository、outbox / job queue 或 compensation audit，不接真实 provider，不处理真实凭证，不做测试连接，不接真实 HIS。
- Phase 23 HIS 连接配置凭证 compensation operation repository 最小边界已完成：当前新增 repository factory 和 repository tests，只操作 `his_connection_credential_compensation_operations` 表，覆盖 create / get、tenant + operationId、tenant + connectionId + operationId、running / succeeded / failed / manual review 状态流转、retry count、pending / stale running 查询、safe operationId 与 provider failure category 白名单、安全 read model 和 repository_error 脱敏；本阶段不接 service / route，不实现 outbox / job queue 或 compensation audit，不修改 audit repository、schema / migration、权限、parser / DTO、provider / storage，不处理真实凭证，不做测试连接，不接真实 HIS。
- Phase 23 HIS 连接配置凭证 compensation outbox / job queue Plan Mode 已完成规划：当前只读盘点确认 operation 表只适合作为补偿状态事实表，不能替代 outbox / job queue；后续需单独规划并实现 outbox / job queue schema / migration、repository、worker claim / lock / stale running recovery、retry、dead letter / manual review、compensation audit integration 和 service 接入；本阶段 docs-only，不修改 `src/**` 或 `drizzle/**`，不实现 outbox / job queue，不处理真实凭证，不做测试连接，不接真实 HIS。
- Phase 23 HIS 连接配置凭证 compensation job queue schema / migration 最小边界已完成：当前新增 `his_connection_credential_compensation_jobs` 单表，承载 outbox 投递语义和 job queue 执行语义；新增 job state / dead letter reason 白名单枚举、operation 表 tenant / connection / operationId 复合唯一约束、job operation 复合外键、job operationId 唯一约束、调度索引、claim / lock 字段、retry / nextAttemptAt 字段和 schema / migration tests；本阶段不实现 repository、不实现 worker / claim / lock runtime、不实现 stale recovery runtime、不接 service、不写 compensation audit、不处理真实凭证、不做测试连接、不接真实 HIS adapter。下一步建议进入 outbox / job queue repository 最小实现或 worker claim / lock Plan Mode。
- Phase 23 HIS 连接配置凭证 compensation job queue repository 最小边界已完成：当前新增 repository factory 和 repository tests，只操作 `his_connection_credential_compensation_jobs` 表，覆盖 create / get、tenant + connection + operationId 隔离、due list、claim / lock、expired lock reclaim、claimVersion 乐观写回、running / succeeded / failed / requeue / dead letter / manual review 状态流转和 repository_error 脱敏；本阶段不新增 schema / migration，不修改 `drizzle/**`，不实现 worker runtime、不接 service、不写 compensation audit、不调用 provider、不处理真实凭证、不做测试连接、不接真实 HIS adapter。下一步建议进入 worker claim / lock / stale recovery Plan Mode 或 compensation audit integration Plan Mode。
- Phase 23 HIS 连接配置凭证 compensation worker claim / lock / stale recovery runtime 最小边界已完成：当前新增 worker 编排 factory，按可信 `tenantId` 调用 due job claim，使用 repository 返回的 `claimId + claimVersion` 写回 job running，并在 job running 成功后推进 operation running；expired claimed job 保守 skipped，expired running job 进入 manual review，stale running operation 找不到 job 或状态不一致时进入 manual review。后续 runner / scheduler、provider test-only execution、retry / dead letter runtime、service 接入、compensation audit integration、observability 和真实 provider / secret manager / HIS adapter 均需单独 PR。本阶段不新增 schema / migration，不修改 `drizzle/**`，不新增 API route，不接 service，不调用 provider，不写 compensation audit，不处理真实凭证，不做测试连接，不接真实 HIS adapter。
- Phase 23 HIS 连接配置凭证 compensation worker test-only provider / no-op execution Plan Mode 已完成：当前只读盘点确认 worker 已具备 provider execution 前置编排入口，operation repository 与 job queue repository 已具备 success / failure / manual review 收口能力；本阶段 docs-only，只规划 test-only provider / no-op execution、provider result 稳定分类、job 与 operation 对齐、claim 写回、事务一致性、audit 边界、租户隔离、safe logging、测试拆分和后续 PR 拆分，不修改 `src/**` 或 `drizzle/**`，不实现 provider runtime，不调用真实 provider，不处理真实凭证，不做测试连接，不接真实 HIS adapter，不新增 runner / scheduler。
- Phase 23 HIS 连接配置凭证 compensation worker no-op provider execution 最小边界已完成：当前 worker 新增可选 `providerExecutor` 和 `executeClaimedCredentialCompensationJob`，provider executor 输入仅包含 tenant / connection / operation、claim、worker 与时间安全上下文；`success` 收口为 job succeeded + operation succeeded，`retryable_failure` 与 `provider_unavailable` 收口为 job failed + operation failed，`unsafe_unknown` 与 `timeout` 收口为 job manual review + operation manual review，`validation_failed` 不调用 executor 且不推进状态。后续 retry / requeue / backoff、dead letter runtime、compensation audit integration、service 接入、runner / scheduler、真实 provider / 测试连接 / 真实 HIS adapter 均需单独 PR。本阶段不新增 schema / migration，不修改 `drizzle/**`，不新增 API route，不接 service，不处理真实凭证，不做测试连接，不接真实 HIS adapter，不写 compensation audit。
- Phase 23 HIS 连接配置凭证 compensation retry / requeue / backoff runtime Plan Mode 已完成：当前只读盘点确认 worker 已有 `retryable_failure` 与 `provider_unavailable` 的 failed 收口入口，job queue repository 已具备 requeue、retryCount、maxRetryCount、nextAttemptAt 与 claimVersion 保护，operation repository 已具备 running / failed / retry count 对齐能力；本阶段 docs-only，只规划 retry policy、requeue、backoff、抖动、retry exhausted 后 dead letter / manual review、job 与 operation 对齐、claim 写回、事务一致性、audit 边界、租户隔离、安全日志、测试拆分和后续 PR 拆分，不修改 `src/**` 或 `drizzle/**`，不新增 schema / migration，不实现 retry / requeue / backoff 或 dead letter / manual review runtime，不接真实 provider，不处理真实凭证，不做测试连接，不接真实 HIS adapter，不新增 runner / scheduler。
- Phase 23 HIS 连接配置凭证 compensation retry policy 纯决策 helper 最小边界已完成：当前新增纯 helper 与测试，输入仅包含 provider result、jobState、retryCount、maxRetryCount、now、baseDelayMs、maxDelayMs、backoffStrategy、jitterMs 和 jitterValue，输出稳定 decision、reason、retryable 与 requeue 时的 nextAttemptAt；helper 不写 repository、不接 worker runtime、不实现 requeue runtime、不实现 dead letter / manual review runtime、不写 compensation audit、不接真实 provider、不处理真实凭证、不做测试连接、不接真实 HIS adapter、不新增 runner / scheduler。下一步建议进入 worker retry / requeue runtime 最小实现或 operation retry count 对齐 Plan Mode。
- Phase 23 HIS 连接配置凭证 compensation worker retry / requeue runtime 最小边界已完成：当前 worker 在 `retryable_failure` 与 `provider_unavailable` 收口为 failed 后调用 retry policy helper，`retryCount < maxRetryCount` 时按 helper 返回的 `nextAttemptAt` 重新排队，requeue 写回绑定 `claimId + claimVersion`，并把 requeue conflict / repository_error 收敛为稳定 worker result；本阶段不新增 schema / migration，不修改 `drizzle/**`，不新增 API route，不接 service，不修改 parser / DTO / provider / storage / 权限 / audit，不接 operation retry count 对齐，不实现 dead letter / manual review runtime，不处理真实凭证，不做测试连接，不接真实 HIS adapter，不新增 runner / scheduler。
- Phase 23 HIS 连接配置凭证 compensation operation retry count 对齐规划已完成：当前仅规划 job retryCount 与 operation retryCount 的职责边界，明确 job retryCount 是调度事实，operation retryCount 是业务状态展示和补偿事实对齐；后续运行时建议在 job requeue 成功后递增 operation retry count，递增失败不回滚 job requeue，允许短暂不一致，并由恢复流程、人工复核和可观测性兜底。本阶段仅文档，不修改 `src/**` 或 `drizzle/**`，不新增 schema / migration，不实现 operation retry count 运行时，不接 dead letter / manual review 运行时、audit、service、route、runner / scheduler、真实 provider、真实凭证、测试连接或真实 HIS adapter。
- Phase 23 HIS 连接配置凭证 compensation operation retry count 对齐 runtime 最小边界已完成：当前 worker 在 job requeue 成功后调用 operation repository 的 `incrementCredentialCompensationOperationRetryCount`，输入继续绑定可信 `tenantId + connectionId + operationId`；requeue 失败、retry exhausted、timeout、unsafe unknown、validation failed 和 retry policy validation failed 均不递增 operation retry count；operation retry count 失败不回滚已完成的 job requeue，并映射为现有稳定 worker result。本阶段不新增 schema / migration，不修改 `drizzle/**`，不修改 job queue repository 或 operation repository，不新增 API route，不接 service、audit、runner / scheduler、真实 provider、真实凭证、测试连接或真实 HIS adapter。
- Phase 23 HIS 连接配置凭证 compensation retry exhausted dead letter runtime Plan Mode 已完成：当前仅规划后续 retry policy 返回 `dead_letter` 时的 job dead letter 写回边界，明确仅适用于 `retryable_failure` / `provider_unavailable` 达到上限，dead letter 写回继续绑定 `tenantId + connectionId + operationId + claimId + claimVersion`，reason 使用现有 `retry_exhausted`；operation 最小边界保持 `compensation_failed`，不递增 operation retry count，不夹带 manual review。本阶段仅文档，不修改 `src/**` 或 `drizzle/**`，不新增 schema / migration，不修改 repository，不接 audit、service、route、runner / scheduler、真实 provider、真实凭证、测试连接或真实 HIS adapter。
- Phase 23 HIS 连接配置凭证 compensation retry exhausted dead letter runtime 最小边界已完成：当前 worker 在 retry policy 返回 `dead_letter` 且 reason 为 `retry_exhausted` 时写回 job dead letter，写回绑定 `tenantId + connectionId + operationId + claimId + claimVersion + now`，reason 固定 `retry_exhausted`；成功后不 requeue、不递增 operation retry count、不推进 manual review，失败时映射为现有 worker result 且不回滚 job failed / operation failed。本阶段不新增 schema / migration，不修改 `drizzle/**`，不修改 job queue repository 或 operation repository，不新增 API route，不接 service、audit、runner / scheduler、真实 provider、真实凭证、测试连接或真实 HIS adapter。
- Phase 23 HIS 连接配置凭证 compensation retry exhausted manual review Plan Mode 已完成：当前仅规划 dead letter 成功后是否推进 operation manual review 的最小边界；只读盘点确认 job 已 `dead_lettered` 后不再调用 job manual review，operation manual review 未来可作为业务终态，但现有 repository 只支持 `compensation_running` 到 `manual_review_required`，不能直接完成 `compensation_failed` 到 `manual_review_required`。本阶段仅文档，不修改 `src/**` 或 `drizzle/**`，不新增 schema / migration，不修改 repository，不实现 manual review runtime，不接 audit、service、route、runner / scheduler、真实 provider、真实凭证、测试连接或真实 HIS adapter；后续需单独进入 operation repository 状态转换边界 Plan Mode 或独立实现 PR。
- Phase 23 HIS 连接配置凭证补偿 operation failed manual review 状态转换边界 Plan Mode 已完成：当前仅规划 operation repository 对 `compensation_failed -> manual_review_required` 的边界；只读盘点确认现有 `markCredentialCompensationOperationManualReviewRequired` 只允许 `compensation_running -> manual_review_required`，会明确拒绝 failed 状态。本阶段建议后续新增 `markFailedCredentialCompensationOperationManualReviewRequired`，只允许 failed 到 manual review，现有 running 方法继续服务 unsafe unknown、timeout 和 stale running recovery。本阶段仅文档，不修改 `src/**` 或 `drizzle/**`，不新增 schema / migration，不修改 operation repository、worker 或 job queue repository，不新增 manual review reason，不接 audit、service、route、runner / scheduler、真实 provider、真实凭证、测试连接或真实 HIS adapter。
- Phase 23 HIS 连接配置凭证补偿 operation failed manual review 状态转换 runtime 最小边界已完成：当前 operation repository 新增 `markFailedCredentialCompensationOperationManualReviewRequired`，只允许 `compensation_failed -> manual_review_required`，写回继续绑定 `tenantId + connectionId + operationId`，不使用 `claimId + claimVersion`，不修改 retryCount 或 lastAttemptAt；现有 running 到 manual review 方法保持不变。本阶段不新增 schema / migration，不修改 `drizzle/**`，不修改 worker 或 job queue repository，不新增 API route，不接 service、audit、runner / scheduler、真实 provider、真实凭证、测试连接或真实 HIS adapter。
- Phase 23 HIS 连接配置凭证补偿 retry exhausted manual review worker runtime 最小边界已完成：当前 worker 在 retry policy 返回 `dead_letter` 且 reason 为 `retry_exhausted`、job dead letter 写回成功后，调用 operation repository 的 `markFailedCredentialCompensationOperationManualReviewRequired`，只绑定 `tenantId + connectionId + operationId`；dead letter 失败不调用 operation manual review，operation manual review 失败不回滚 job dead letter、不 requeue、不递增 operation retry count，并返回现有稳定 worker result。本阶段不新增 schema / migration，不修改 `drizzle/**`，不修改 operation repository 或 job queue repository，不新增 API route，不接 service、audit、runner / scheduler、真实 provider、真实凭证、测试连接或真实 HIS adapter。
...
- Phase 23 HIS 连接配置状态 service audit reason 映射：已完成 docs-only spec / plan，状态 service 实现前置 reason 已收敛；后续状态 service 只能使用既有 `allowed_by_policy`、`not_found_or_not_owned`、`invalid_transition` 和 `invalid_his_connection_payload`，不得新增状态专用 reason 或把 repository error message、database constraint、status、credentialRef、healthStatus、raw payload 写入 audit。
...
- 治疗记录模块：结构化摘要 v1、租户隔离、人工录入 v1、只读管理 v1、治疗后护理 / 随访联动 v1、随访任务来源治理增强 v1、标准治疗事件 domain-only 契约、治疗摘要编辑能力 v1、治疗摘要作废能力 v1、治疗项目路径模板 v1、随访路径运营分析 v1 最小闭环、HIS 标准治疗事件 mapper v1 Plan Mode / 契约差异评估 / 缺口字段 domain-only 契约 / 解析器安全测试 / 文档 smoke 收尾、真实 HIS adapter 前置评估、连接配置与凭证边界、连接配置 schema / API Plan Mode、连接配置 schema / migration 最小实现、连接配置只读 repository 最小实现、连接配置 list / detail 只读 API 最小实现、连接配置只读 UI / workspace 入口轻量实现、Phase 23 写入 API 与状态流转边界 Plan Mode、Phase 23 写入 repository 边界 Plan Mode、Phase 23 create / update repository 最小实现、Phase 23 状态流转 repository 最小实现、Phase 23 repository 写入闭环收尾、Phase 23 写入 parser / 权限 / service / audit reason / service denied audit、Phase 23 create / update API route Plan Mode、Phase 23 状态 API / 权限 / service / service audit reason Plan Mode、Phase 23 状态 API route denied audit Plan Mode 和 Phase 23 状态 API 闭环收尾均已完成；后续版本历史 / diff 展示、客服会话联动、完整治疗记录能力、凭证管理 Plan Mode、凭证加密与密钥管理、连接健康检查 / 测试连接、真实 HIS adapter spec / plan、Webhook / 同步任务、患者身份匹配、人工复核 / 预览、adapter domain-only 输入 DTO / parser、路径模板 schema / API、随访路径图表 / 导出 / 归因 / 路径效果分析和外部系统接入需单独规划，仍不保存完整病历正文。
...
3. HIS 标准治疗事件 mapper / 真实 HIS adapter 后续评估：Phase 22 已完成 Plan Mode、契约差异评估、缺口字段 domain-only 契约补齐、解析器安全测试和文档 / smoke 收尾；真实 HIS adapter 前置评估、连接配置与凭证边界、连接配置 schema / API Plan Mode、连接配置 schema / migration 最小实现、连接配置只读 repository 最小实现、连接配置 list / detail 只读 API 最小实现、连接配置只读 UI / workspace 入口轻量实现、Phase 23 写入 API 与状态流转边界 Plan Mode、Phase 23 写入 repository 边界 Plan Mode、Phase 23 create / update repository 最小实现、Phase 23 状态流转 repository 最小实现、Phase 23 repository 写入闭环收尾、写入 parser / 权限 / service / audit reason / service denied audit、create / update API route Plan Mode、状态 API / 权限 / service Plan Mode、状态 service audit reason 映射、状态 API route denied audit Plan Mode、状态 API 闭环收尾、凭证管理总边界 Plan Mode、凭证 repository / storage 最小边界、凭证 parser / service / DTO 最小实现、凭证 API route / permission / audit Plan Mode、凭证 API route / permission / audit 最小实现、provider failure audit / service audit 最小边界、compensation audit Plan Mode、compensation metadata / outbox Plan Mode、compensation metadata / operationId schema 最小边界、compensation operation repository Plan Mode、compensation operation repository 最小边界、compensation outbox / job queue Plan Mode、compensation job queue schema / migration 最小边界、compensation job queue repository 最小边界、compensation worker claim / lock / stale recovery Plan Mode、compensation worker claim / lock / stale recovery runtime 最小边界、compensation worker test-only provider / no-op execution Plan Mode、compensation worker no-op provider execution 最小边界、retry / requeue / backoff Plan Mode、retry policy helper、worker retry / requeue runtime、operation retry count 对齐规划与运行时、retry exhausted dead letter Plan Mode 与运行时、retry exhausted manual review Plan Mode、operation failed manual review 状态转换边界 Plan Mode 与 runtime、retry exhausted manual review worker runtime 均已完成。下一步如进入 dead letter / manual review recovery、compensation audit repository / service integration、service 接入 compensation operation + outbox 写入、runner / scheduler、真实凭证处理 Plan Mode、凭证加密与外部密钥管理系统、连接健康检查 / 测试连接、真实 HIS adapter spec / plan、Webhook / 同步任务、患者身份匹配、人工复核 / 预览、adapter domain-only 输入 DTO / parser、治疗摘要创建来源治理或真实外部系统接入 PoC，仍需单独规划或独立 PR，不接真实 HIS、不保存外部系统原始交互内容、不自动摘要、不自动任务、不自动触达。
...
- Phase 23 HIS 连接配置状态 API route denied audit Plan Mode 已完成：新增 spec / plan 文档，只规划后续 pause / resume / revoke / DELETE route 层权限拒绝和 parser 失败 denied audit，明确 `manage_status` / `delete` action、既有 reason 复用、path `connectionId` resourceId、audit 失败 fail closed、禁止重复 status service 审计和敏感信息禁区；仍不新增 API、不修改 `src/**`、不新增 audit reason / action、不处理凭证、不做测试连接、不接真实 HIS、不修改 demo seed。
- Phase 23 HIS 连接配置状态 API 已闭环：状态权限、状态 service、pause / resume route、revoke / DELETE route、403 权限拒绝 route denied audit、parser 失败 route denied audit、route tests、status service tests 和权限 / audit domain 回归均已完成；状态 API 成功响应仍固定 `{ ok: true }`，错误响应只返回稳定 code / error，不返回凭证、状态 read model、租户字段、actor 字段或敏感信息；凭证管理、测试连接和真实 HIS adapter 仍未开始，下一步建议先进入凭证管理 Plan Mode。
