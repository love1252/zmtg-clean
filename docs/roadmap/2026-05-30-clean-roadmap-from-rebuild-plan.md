...
- 审计日志完整查询页面、治疗记录、客服会话、知识库、AI、企业微信、开放平台凭证和计费仍未进入真实实现。
...
- 审计日志只读查询：平台端看聚合和安全事件，机构端看本租户操作事件。
...
1. Phase 8 Plan Mode：审计日志只读查询基础版。
2. 治疗记录 schema、API 和安全计划。
3. 平台租户管理与套餐权益。
4. 知识库/RAG 基础版。
5. AI provider、调用日志和 Agent。
6. 企业微信、Webhook、OAuth、API Key。
7. 计费、合同、发票和支付。
...
- 后续建议先进入 Phase 8 Plan Mode，单独评估审计日志只读查询基础版的机构 / 平台可见范围、筛选白名单、分页、错误脱敏和保留策略。
