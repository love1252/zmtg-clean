...
- 随访路径运营分析第二十一阶段 Plan Mode：Phase 21 spec / plan 文档规划已完成，仅定义基于治疗摘要、路径模板建议、来源随访任务、任务状态和审计记录的最小分析口径，不进入功能实现。
- 随访路径运营分析第二十一阶段 PR 2：domain-only 纯函数口径和单元测试已完成，仍未新增 API、UI、schema / migration、权限或外部系统接入。
...
- 治疗记录结构化摘要 v1、治疗摘要人工录入 v1、治疗摘要管理 v1、治疗后护理 / 随访联动 v1、随访任务来源治理增强 v1、标准治疗事件 domain-only 契约、治疗摘要编辑能力 v1、治疗摘要作废能力 v1 和治疗项目路径模板 v1 已完成；随访路径运营分析 v1 已完成 docs-only 口径规划和 domain-only 纯函数口径，但未进入 API / UI / 报表实现；真实 HIS 接入、Webhook、文件导入、外部系统同步、完整治疗记录正文、版本历史 / diff 展示、客服会话、知识库、AI、企业微信、开放平台凭证和计费仍未进入真实实现。
...
- 治疗记录模块：结构化摘要 v1、租户隔离、人工录入 v1、只读管理 v1、治疗后护理 / 随访联动 v1、随访任务来源治理增强 v1、标准治疗事件 domain-only 契约、治疗摘要编辑能力 v1、治疗摘要作废能力 v1 和治疗项目路径模板 v1 已完成；随访路径运营分析 v1 已完成 Plan Mode 口径规划和 domain-only 纯函数评估；后续版本历史 / diff 展示、客服会话联动、完整治疗记录能力、路径模板 schema / API、随访路径 API / UI 实现和外部系统接入需单独规划，仍不保存完整病历正文。
...
5. 随访路径运营分析 v1：Phase 21 Plan Mode 和 PR 2 domain-only 口径已完成；后续如进入 API / UI 实现，必须继续单独评估，不做复杂归因模型、图表导出或自动触达。
...
- Phase 21 随访路径运营分析 v1 Plan Mode 已完成：spec / plan 明确当前只是 docs-only 口径规划，目标是基于现有治疗摘要、路径模板建议、来源随访任务、任务状态和审计记录定义模板建议数、人工确认任务数、任务完成数、任务超时数、作废摘要阻断数和重复来源任务冲突数。
- Phase 21 当前未新增 API route，未改 DTO、数据库 schema / migration、权限、认证或租户隔离，未接 HIS / 企微 / AI / RAG / Agent，未做自动触达，未做复杂经营智能中心、图表 UI、报表导出、经营归因或 demo seed 修改。
- 后续如需分析指标落库、报表 API、图表 UI、导出、经营归因、审计口径补强或外部系统接入，必须单独 Plan Mode。
- Phase 21 PR 2 随访路径运营分析 domain-only 口径已完成：新增纯函数和 institution 单元测试，基于现有治疗摘要、模板建议、来源随访任务、固定 `analysisAt` 和审计事件计算六个最小指标。
- Phase 21 PR 2 未新增 API route，未改现有 API 路径或 DTO 字段，未改 UI、schema / migration、权限、认证或租户隔离，未接 HIS / 企微 / AI / RAG / Agent，未做自动触达、经营智能中心、图表 UI、报表导出、经营归因或 demo seed 修改。
