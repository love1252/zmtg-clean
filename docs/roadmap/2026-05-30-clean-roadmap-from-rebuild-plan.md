...
- Phase 23 HIS 连接配置凭证 compensation worker claim / lock / stale recovery Plan Mode 已完成规划：当前只读盘点确认 operation repository 足以支撑补偿事实状态推进，job queue repository 足以支撑 claim / lock、expired lock 发现、retry / requeue、dead letter 和 manual review 状态写回；后续 worker runtime、runner / scheduler、provider test-only execution、stale recovery runtime、retry / dead letter / manual review runtime、service 接入和 compensation audit integration 均需单独 PR。本阶段 docs-only，不修改 `src/**` 或 `drizzle/**`，不新增 schema / migration，不实现 worker runtime，不调用 provider，不接 service，不处理真实凭证，不做测试连接，不接真实 HIS adapter。
