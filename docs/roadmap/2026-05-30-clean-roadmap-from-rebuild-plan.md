...
- Phase 23 HIS 连接配置凭证 compensation job queue schema / migration 最小边界已完成：当前新增 `his_connection_credential_compensation_jobs` 单表，承载 outbox 投递语义和 job queue 执行语义；新增 job state / dead letter reason 白名单枚举、operationId 唯一约束、tenant / connection / operationId 关联、调度索引、claim / lock 字段、retry / nextAttemptAt 字段和 schema / migration tests；本阶段不实现 repository、不实现 worker / claim / lock runtime、不实现 stale recovery runtime、不接 service、不写 compensation audit、不处理真实凭证、不做测试连接、不接真实 HIS adapter。下一步建议进入 outbox / job queue repository 最小实现或 worker claim / lock Plan Mode。
