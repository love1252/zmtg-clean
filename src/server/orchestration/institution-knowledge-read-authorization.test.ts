...
import {
  ...
  isInstitutionKnowledgeReadAuthorizationHandleV1,
  resolveInstitutionKnowledgeReadAuthorizationV1,
  type InstitutionKnowledgeReadAuthorizationResolutionV1,
...
describe('Knowledge document metadata formal read authorization', () => {
  it('无 caller scope 输入，并使用 formal identity/membership/binding/scope chain', async () => {
    expectTypeOf<
      Parameters<typeof resolveInstitutionKnowledgeReadAuthorizationV1>
    >().toEqualTypeOf<[]>();

    expectTypeOf<
      ReturnType<typeof resolveInstitutionKnowledgeReadAuthorizationV1>
    >().toEqualTypeOf<
      Promise<InstitutionKnowledgeReadAuthorizationResolutionV1>
    >();
  ...
  it.each(['tenant_admin', 'tenant_operator'] as const)(
    '%s 可以获得 dedicated Knowledge readonly authorization',
    async (role) => {
  ...
  it.each(['consultant', 'customer_service'] as const)(
    '%s 即使被伪造为 knowledge section audience 也不能读取 Knowledge metadata',
    async (role) => {
      mocks.consumeSnapshot.mockReset();
      mocks.resolveForSession.mockReset();
      resolveRole(role);
  ...
  it('knowledge section audience denial 直接 forbidden', async () => {
  ...
  it('plain、clone 与 Proxy 都不能伪造 authorization handle', () => {
    for (const value of [
      ...
      expect(
        isInstitutionKnowledgeReadAuthorizationHandleV1(value),
      ).toBe(false);
      expect(
        consumeInstitutionKnowledgeReadAuthorizationV1(value),
      ).toBeNull();
    }
  });
});
