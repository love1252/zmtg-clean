...
import {
  ...
  isInstitutionCustomerWriteAuthorizationHandleV1,
  resolveInstitutionCustomerWriteAuthorizationV1,
  type InstitutionCustomerWriteAuthorizationResolutionV1,
...
describe('Customer formal write authorization', () => {
  it('has no caller input and resolves only from formal provenance chain', async () => {
    expectTypeOf<
      Parameters<typeof resolveInstitutionCustomerWriteAuthorizationV1>
    >().toEqualTypeOf<[]>();
    expectTypeOf<
      ReturnType<typeof resolveInstitutionCustomerWriteAuthorizationV1>
    >().toEqualTypeOf<
      Promise<InstitutionCustomerWriteAuthorizationResolutionV1>
    >();
  ...
  it.each(institutionRoles)(
    ...
    async (role) => {
      ...
      const result =
        await resolveInstitutionCustomerWriteAuthorizationV1();
  ...
  it('plain, clone, and Proxy cannot forge the handle', () => {
    for (const value of [
      ...
      expect(
        isInstitutionCustomerWriteAuthorizationHandleV1(value),
      ).toBe(false);
      expect(
        consumeInstitutionCustomerWriteAuthorizationV1(value),
      ).toBeNull();
    }
  });
});
