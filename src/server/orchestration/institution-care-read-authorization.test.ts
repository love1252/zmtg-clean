...
vi.mock(
  ...
  async (importOriginal) => ({
    ...(await importOriginal<
      typeof import('@/modules/institution-contracts/v1/institution-navigation')
    >()),
    isInstitutionRoleV1: mocks.isInstitutionRole,
    isRoleInInstitutionSectionAudienceV1: mocks.isAudienceAllowed,
  }),
);
vi.mock(
  ...
  async (importOriginal) => ({
    ...(await importOriginal<
      typeof import('@/modules/security/server/institution-action-policy')
    >()),
    createInstitutionActionPolicyV1: () =>
      Object.freeze({ authorize: mocks.authorizeAction }),
    isInstitutionActionPolicyAllowV1: mocks.isPolicyAllow,
  }),
);

import {
  consumeInstitutionCareReadAuthorizationV1,
  isInstitutionCareReadAuthorizationHandleV1,
  resolveInstitutionCareReadAuthorizationV1,
  type InstitutionCareReadAuthorizationResolutionV1,
...
const formalSnapshot = Object.freeze({ opaque: 'snapshot' });
...
beforeEach(() => {
  ...
  mocks.authorizeAction.mockReturnValue(
    Object.freeze({ kind: 'allowed', objectType: 'care_task', action: 'read' }),
  );
  mocks.isPolicyAllow.mockReturnValue(true);
  resolveRole('tenant_admin');
});

describe('Care formal read authorization', () => {
  it('无 caller 输入并使用 formal provenance → identity/membership/scope chain', async () => {
    expectTypeOf<Parameters<typeof resolveInstitutionCareReadAuthorizationV1>>()
      .toEqualTypeOf<[]>();
    expectTypeOf<ReturnType<typeof resolveInstitutionCareReadAuthorizationV1>>()
      .toEqualTypeOf<Promise<InstitutionCareReadAuthorizationResolutionV1>>();

    await expect(resolveInstitutionCareReadAuthorizationV1()).resolves.toMatchObject({
      kind: 'allowed',
    });
  ...
  it.each(institutionRoles)(
    '%s 通过 care section + care_task/read policy',
    async (role) => {
      mocks.consumeSnapshot.mockReset();
      mocks.resolveForSession.mockReset();
      resolveRole(role);

      const result = await resolveInstitutionCareReadAuthorizationV1();
      expect(result.kind).toBe('allowed');
      expect(mocks.isAudienceAllowed).toHaveBeenCalledWith(role, 'care');
  ...
  it('签发 opaque、frozen、genuine 且 exactly-once 的 formal pair handle', async () => {
    const result = await resolveInstitutionCareReadAuthorizationV1();
    ...
    expect(isInstitutionCareReadAuthorizationHandleV1(result.authorization)).toBe(true);
    expect(consumeInstitutionCareReadAuthorizationV1(result.authorization)).toEqual({
      tenantId: 'tenant-001',
      institutionId: 'institution-001',
      observedAt: '2026-08-16T08:00:00.000Z',
    });
    expect(consumeInstitutionCareReadAuthorizationV1(result.authorization)).toBeNull();
  });

  it('wrong section audience 与 action denial 都 forbidden', async () => {
    mocks.isAudienceAllowed.mockReturnValueOnce(false);
    await expect(resolveInstitutionCareReadAuthorizationV1()).resolves.toEqual({
      kind: 'forbidden',
    });
  ...
  it.each(['platform_admin', 'unknown_role'])(
    '%s 不能伪装 institution role',
    async (role) => {
      mocks.consumeSnapshot.mockReset();
      mocks.resolveForSession.mockReset();
      resolveRole(role);
  ...
  it.each(['denied', 'invalid', 'unavailable', 'stale'])(
    'invalid membership/scope context %s fail-closed',
    async (kind) => {
      mocks.resolveForSession.mockReset();
  ...
  it('binding/session/scope pair 或 membership role mismatch fail-closed', async () => {
    mocks.consumeSnapshot.mockReset();
    mocks.resolveForSession.mockReset();
  ...
  it('invalid/mixed session 与异常统一低敏 unavailable', async () => {
    const mixed =
      'zmtg_demo_session=demo; zmtg_formal_session_v1=a; zmtg_formal_session_v1=b';
  ...
  it('plain/clone/Proxy 不能伪造 handle', () => {
    for (const value of [{}, Object.freeze({}), new Proxy({}, {})]) {
      expect(isInstitutionCareReadAuthorizationHandleV1(value)).toBe(false);
      expect(consumeInstitutionCareReadAuthorizationV1(value)).toBeNull();
    }
  });
});
