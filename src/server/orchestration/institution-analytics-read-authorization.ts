...
declare const institutionAnalyticsReadAuthorizationMarkerV1: unique symbol;

export type InstitutionAnalyticsReadAuthorizationHandleV1 = Readonly<{
  readonly [institutionAnalyticsReadAuthorizationMarkerV1]:
    'institution_analytics_read_authorization_v1';
}>;

export type InstitutionAnalyticsReadAuthorizationConsumptionV1 = Readonly<{
  tenantId: string;
  institutionId: string;
  observedAt: string;
}>;

export type InstitutionAnalyticsReadAuthorizationResolutionV1 =
  | Readonly<{
      kind: 'allowed';
      authorization: InstitutionAnalyticsReadAuthorizationHandleV1;
    }>
...
const consumptions = new WeakMap<
  object,
  InstitutionAnalyticsReadAuthorizationConsumptionV1
>();
...
export function isInstitutionAnalyticsReadAuthorizationHandleV1(
  value: unknown,
): value is InstitutionAnalyticsReadAuthorizationHandleV1 {
  try {
    return (
      value !== null &&
      typeof value === 'object' &&
...
export function consumeInstitutionAnalyticsReadAuthorizationV1(
  value: unknown,
): InstitutionAnalyticsReadAuthorizationConsumptionV1 | null {
  if (!isInstitutionAnalyticsReadAuthorizationHandleV1(value)) return null;
...
export async function resolveInstitutionAnalyticsReadAuthorizationV1(): Promise<InstitutionAnalyticsReadAuthorizationResolutionV1> {
  try {
    ...
    if (!isRoleInInstitutionSectionAudienceV1(user.role, 'analytics')) {
      return FORBIDDEN;
    }
    ...
    }) satisfies InstitutionAnalyticsReadAuthorizationConsumptionV1;
    const authorization = Object.freeze(
      {},
    ) as InstitutionAnalyticsReadAuthorizationHandleV1;
