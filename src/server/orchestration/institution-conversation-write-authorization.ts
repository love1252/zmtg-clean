...
declare const institutionConversationWriteAuthorizationMarkerV1: unique symbol;

export type InstitutionConversationWriteAuthorizationHandleV1 = Readonly<{
  readonly [institutionConversationWriteAuthorizationMarkerV1]:
    'institution_conversation_write_authorization_v1';
}>;

export type InstitutionConversationWriteAuthorizationConsumptionV1 = Readonly<{
  accountId: string;
  displayName: string;
...
export type InstitutionConversationWriteAuthorizationResolutionV1 =
  | Readonly<{
      kind: 'allowed';
      authorization: InstitutionConversationWriteAuthorizationHandleV1;
    }>
...
const consumptions = new WeakMap<
  object,
  InstitutionConversationWriteAuthorizationConsumptionV1
>();
...
export function isInstitutionConversationWriteAuthorizationHandleV1(
  value: unknown,
): value is InstitutionConversationWriteAuthorizationHandleV1 {
  try {
    return (
      value !== null
      && typeof value === 'object'
...
export function consumeInstitutionConversationWriteAuthorizationV1(
  value: unknown,
): InstitutionConversationWriteAuthorizationConsumptionV1 | null {
  if (!isInstitutionConversationWriteAuthorizationHandleV1(value)) return null;
...
export async function resolveInstitutionConversationWriteAuthorizationV1(): Promise<
  InstitutionConversationWriteAuthorizationResolutionV1
> {
  try {
    ...
    if (!isRoleInInstitutionSectionAudienceV1(user.role, 'conversations')) {
      return FORBIDDEN;
    }
    ...
    if (
      ...
      || action.objectType !== 'conversation'
    ...
    }) satisfies InstitutionConversationWriteAuthorizationConsumptionV1;

    const authorization = Object.freeze(
      {},
    ) as InstitutionConversationWriteAuthorizationHandleV1;
