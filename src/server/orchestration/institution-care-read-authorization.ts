...
declare const institutionCareReadAuthorizationMarkerV1: unique symbol;

export type InstitutionCareReadAuthorizationHandleV1 = Readonly<{
  readonly [institutionCareReadAuthorizationMarkerV1]:
    'institution_care_read_authorization_v1';
}>;

export type InstitutionCareReadAuthorizationConsumptionV1 = Readonly<{
  tenantId: string;
  institutionId: string;
  observedAt: string;
}>;

export type InstitutionCareReadAuthorizationResolutionV1 =
  | Readonly<{
      kind: 'allowed';
      authorization: InstitutionCareReadAuthorizationHandleV1;
    }>
...
const consumptions = new WeakMap<
  object,
  InstitutionCareReadAuthorizationConsumptionV1
>();
...
export function isInstitutionCareReadAuthorizationHandleV1(
  value: unknown,
): value is InstitutionCareReadAuthorizationHandleV1 {
  try {
    return (
      value !== null &&
      typeof value === 'object' &&
...
export function consumeInstitutionCareReadAuthorizationV1(
  value: unknown,
): InstitutionCareReadAuthorizationConsumptionV1 | null {
  if (!isInstitutionCareReadAuthorizationHandleV1(value)) return null;
...
export async function resolveInstitutionCareReadAuthorizationV1(): Promise<InstitutionCareReadAuthorizationResolutionV1> {
  try {
    ...
    }) satisfies InstitutionCareReadAuthorizationConsumptionV1;
    const authorization = Object.freeze(
      {},
    ) as InstitutionCareReadAuthorizationHandleV1;
