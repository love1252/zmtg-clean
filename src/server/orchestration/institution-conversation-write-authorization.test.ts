...
import {
  ...
  isInstitutionConversationWriteAuthorizationHandleV1,
  resolveInstitutionConversationWriteAuthorizationV1,
  type InstitutionConversationWriteAuthorizationResolutionV1,
...
describe('Conversation formal write authorization', () => {
  it('has no caller input and resolves only from formal provenance chain', async () => {
    expectTypeOf<
      Parameters<typeof resolveInstitutionConversationWriteAuthorizationV1>
    >().toEqualTypeOf<[]>();
    expectTypeOf<
      ReturnType<typeof resolveInstitutionConversationWriteAuthorizationV1>
    >().toEqualTypeOf<
      Promise<InstitutionConversationWriteAuthorizationResolutionV1>
    >();
  ...
  it.each(institutionRoles)(
    ...
    async (role) => {
      ...
      const result =
        await resolveInstitutionConversationWriteAuthorizationV1();
  ...
  it('plain, clone, and Proxy cannot forge the handle', () => {
    for (const value of [
      ...
      expect(
        isInstitutionConversationWriteAuthorizationHandleV1(value),
      ).toBe(false);
      expect(
        consumeInstitutionConversationWriteAuthorizationV1(value),
      ).toBeNull();
    }
  });
});
