...
describe('POST-V2-R1C page_system_audit readonly release authority', () => {
  ...
  it('tenant_admin receives five operational pages, four readonly pages and three controlled-create actions', async () => {
    ...
    const customerCreate = capabilities.find(
    ...
    const careFollowupCreate = capabilities.find(
    ...
    expect(careFollowupCreate).toEqual({
  ...
  it.each(['consultant', 'customer_service'] as const)(
    ...
    async (role) => {
      ...
      expect(
        capabilities.find((item) => item.key === 'page_care_followups'),
      ).toMatchObject({
        decision: 'operational',
        dimensions: {
          codeMaturity: 'verified',
  ...
  it('tenant_operator receives the exact page_system_audit readonly partial pilot shape without making Authority a role source', async () => {
    runtimeMocks.membershipRead.mockResolvedValue([
      {
        ...membershipRow,
  ...
  it('releases customer, appointment and follow-up controlled-create', async () => {
    const status = await resolveInstitutionCapabilityAuthorityStatusV1();
    const byKey = new Map(
