...
declare const institutionAiUsageReadAuthorizationMarkerV1: unique symbol;

export type InstitutionAiUsageReadAuthorizationHandleV1 = Readonly<{
  readonly [institutionAiUsageReadAuthorizationMarkerV1]:
    'institution_ai_usage_read_authorization_v1';
}>;

export type InstitutionAiUsageReadAuthorizationConsumptionV1 = Readonly<{
  tenantId: string;
  institutionId: string;
  observedAt: string;
}>;

export type InstitutionAiUsageReadAuthorizationResolutionV1 =
  | Readonly<{
      kind: 'allowed';
      authorization: InstitutionAiUsageReadAuthorizationHandleV1;
    }>
...
const consumptions = new WeakMap<
  object,
  InstitutionAiUsageReadAuthorizationConsumptionV1
>();
...
export function isInstitutionAiUsageReadAuthorizationHandleV1(
  value: unknown,
): value is InstitutionAiUsageReadAuthorizationHandleV1 {
  try {
    return (
      value !== null &&
      typeof value === 'object' &&
...
export function consumeInstitutionAiUsageReadAuthorizationV1(
  value: unknown,
): InstitutionAiUsageReadAuthorizationConsumptionV1 | null {
  if (!isInstitutionAiUsageReadAuthorizationHandleV1(value)) return null;
...
export async function resolveInstitutionAiUsageReadAuthorizationV1(): Promise<InstitutionAiUsageReadAuthorizationResolutionV1> {
  try {
    ...
    if (!isRoleInInstitutionSectionAudienceV1(user.role, 'system')) {
      return FORBIDDEN;
    }
    ...
    }) satisfies InstitutionAiUsageReadAuthorizationConsumptionV1;
    const authorization = Object.freeze(
      {},
    ) as InstitutionAiUsageReadAuthorizationHandleV1;
