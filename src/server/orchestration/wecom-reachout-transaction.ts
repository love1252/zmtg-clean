...
import {
  ...
  type WeComRealSendProofTransactionRepository,
} from '@/modules/institution/server/wecom-real-send-proof-repository';
import {
  createWeComReachOutCommandRepository,
} from '@/modules/messaging/server/wecom-reachout-command-repository';
...
function createCanonicalSafetyRepository(
...
export async function runWeComReachOutTransaction<T>(
  database: TenantDatabase,
  operation: (
    dependencies: WeComReachOutTransactionDependencies,
  ) => Promise<T>,
): Promise<T> {
