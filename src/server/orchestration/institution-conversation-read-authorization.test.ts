...
import {
  ...
  isInstitutionConversationReadAuthorizationHandleV1,
  resolveInstitutionConversationReadAuthorizationV1,
  type InstitutionConversationReadAuthorizationResolutionV1,
...
describe('Conversations Queue formal read authorization', () => {
  it('无 caller 输入，只使用 formal session provenance 与 authoritative context', async () => {
    expectTypeOf<Parameters<typeof resolveInstitutionConversationReadAuthorizationV1>>()
      .toEqualTypeOf<[]>();
    expectTypeOf<ReturnType<typeof resolveInstitutionConversationReadAuthorizationV1>>()
      .toEqualTypeOf<Promise<InstitutionConversationReadAuthorizationResolutionV1>>();

    await expect(resolveInstitutionConversationReadAuthorizationV1()).resolves.toMatchObject({
      kind: 'allowed',
    });
  ...
  it.each([
    ...
    'consultant',
    'customer_service',
  ] as const)('%s 通过 conversations section + conversation/read policy', async (role) => {
    mocks.consumeSnapshot.mockReset();
    mocks.resolveForSession.mockReset();
    resolveRole(role);

    const result = await resolveInstitutionConversationReadAuthorizationV1();
    expect(result.kind).toBe('allowed');
    if (result.kind !== 'allowed') throw new Error('expected allowed');
    expect(isInstitutionConversationReadAuthorizationHandleV1(result.authorization)).toBe(true);
  });
  ...
  it('签发冻结、opaque、genuine 且 exactly-once 的 pair handle', async () => {
    ...
    expect(consumeInstitutionConversationReadAuthorizationV1(result.authorization)).toBeNull();
    expect(isInstitutionConversationReadAuthorizationHandleV1(result.authorization)).toBe(false);
  });

  it.each(['platform_admin', 'unknown_role'])('%s 不能进入 Conversations', async (role) => {
    mocks.consumeSnapshot.mockReset();
    mocks.resolveForSession.mockReset();
    resolveRole(role);
    await expect(resolveInstitutionConversationReadAuthorizationV1()).resolves.toEqual({
      kind: 'forbidden',
    });
  });
  ...
  it('plain/clone/Proxy 不能伪造 handle，异常保持低敏 unavailable', async () => {
    for (const value of [{}, Object.freeze({}), new Proxy({}, {})]) {
      expect(isInstitutionConversationReadAuthorizationHandleV1(value)).toBe(false);
      expect(consumeInstitutionConversationReadAuthorizationV1(value)).toBeNull();
    }
    mocks.resolveForSession.mockReset();
