...
const updateKnowledgeTableCard=(card,signature,markup)=>{if(card.dataset.previewKnowledgeSignature===signature)return;card.dataset.previewKnowledgeSignature=signature;card.innerHTML=markup};
const knowledgeCategories=()=>['全部知识',...new Set(knowledgeDocumentRuntime.records.map(record=>String(record.sourceLabel||'').trim()).filter(Boolean))];
const visibleKnowledgeRecords=()=>state.knowledgeTab==='知识文档'?knowledgeDocumentRuntime.records.filter(record=>state.knowledgeCategory==='全部知识'||record.sourceLabel===state.knowledgeCategory):[];
const renderKnowledgeNavigation=()=>{if(state.route!=='/knowledge'||!knowledgeDocumentRuntime.loaded)return;const categories=knowledgeCategories();if(!categories.includes(state.knowledgeCategory))state.knowledgeCategory='全部知识';document.querySelectorAll('[data-knowledge-tab]').forEach(button=>{const type=button.dataset.knowledgeTab||'';const count=type==='知识文档'?knowledgeDocumentRuntime.records.length:0;const badge=button.querySelector('.tab-count');if(badge)setTextIfChanged(badge,String(count));button.classList.toggle('active',type===state.knowledgeTab);button.setAttribute('aria-pressed',type===state.knowledgeTab?'true':'false')});const tree=[...document.querySelectorAll('#page .tree')].find(element=>element.querySelector('[data-knowledge-category]'));if(!tree)return;const treeSignature=JSON.stringify([state.knowledgeCategory,categories,knowledgeDocumentRuntime.records.map(record=>record.sourceLabel)]);if(tree.dataset.previewKnowledgeCategorySignature===treeSignature)return;tree.dataset.previewKnowledgeCategorySignature=treeSignature;tree.querySelectorAll('[data-knowledge-category]').forEach(button=>button.remove());const rule=tree.querySelector('.rule');const markup=categories.map(category=>{const count=category==='全部知识'?knowledgeDocumentRuntime.records.length:knowledgeDocumentRuntime.records.filter(record=>record.sourceLabel===category).length;return '<button class="tree-item '+(state.knowledgeCategory===category?'active':'')+'" data-knowledge-category="'+esc(category)+'" aria-pressed="'+(state.knowledgeCategory===category?'true':'false')+'">'+ico(category==='全部知识'?'folder':'file')+' <span>'+esc(category)+'</span><span class="tab-count">'+count+'</span></button>'}).join('');if(rule)rule.insertAdjacentHTML('beforebegin',markup);else tree.insertAdjacentHTML('beforeend',markup)};
const knowledgeEmptyState=()=>{if(state.knowledgeTab!=='知识文档')return '<tr><td colspan="9"><div class="preview-knowledge-table-state"><div><b>当前机构暂无已发布'+esc(state.knowledgeTab)+'内容</b><span>该分栏已接入正式数据筛选，不再展示原型固定数量。</span></div></div></td></tr>';if(state.knowledgeCategory!=='全部知识')return '<tr><td colspan="9"><div class="preview-knowledge-table-state"><div><b>“'+esc(state.knowledgeCategory)+'”暂无已发布知识</b><span>发布到该分类后会自动显示在这里。</span></div></div></td></tr>';return '<tr><td colspan="9"><div class="preview-knowledge-table-state"><div><b>当前机构暂无已发布知识</b><span>上传并完成解析确认、发布后会显示在这里。</span></div></div></td></tr>'};
const renderPublishedKnowledge=()=>{if(state.route!=='/knowledge')return;const card=knowledgeTableCard();if(!card)return;card.classList.add('preview-knowledge-runtime-card');if(knowledgeDocumentRuntime.loading){updateKnowledgeTableCard(card,'loading','<div class="preview-knowledge-table-state"><div><b>正在读取正式知识</b><span>仅展示当前机构已发布的不可变版本。</span></div></div>');return}if(knowledgeDocumentRuntime.error){updateKnowledgeTableCard(card,'error:'+knowledgeDocumentRuntime.error,'<div class="preview-knowledge-table-state"><div><b>正式知识暂不可用</b><span>'+esc(knowledgeDocumentRuntime.error)+'</span></div></div>');return}if(!knowledgeDocumentRuntime.loaded)return;renderKnowledgeNavigation();const visibleRecords=visibleKnowledgeRecords();const rows=visibleRecords.length?visibleRecords.map(record=>'<tr><td><button class="link">'+esc(record.title)+'</button></td><td>知识文档</td><td>'+esc(record.sourceLabel)+'</td><td><b>V'+Number(record.version)+'</b></td><td>'+tag('已发布','green')+'</td><td><span class="muted">待授权</span></td><td>'+tag('人工确认','orange')+'</td><td>—</td><td><span class="muted">'+new Date(record.publishedAt).toLocaleDateString('zh-CN')+'</span></td></tr>').join(''):knowledgeEmptyState();const signature='ready:'+state.knowledgeTab+':'+state.knowledgeCategory+':'+JSON.stringify(knowledgeDocumentRuntime.records.map(record=>[record.documentId,record.version,record.title,record.sourceLabel,record.publishedAt]));updateKnowledgeTableCard(card,signature,'<table class="table"><thead><tr><th>标题</th><th>类型</th><th>分类</th><th>版本</th><th>状态</th><th>可调用AI</th><th>发布方式</th><th>引用</th><th>发布时间</th></tr></thead><tbody>'+rows+'</tbody></table><div class="table-foot"><span>当前显示 '+visibleRecords.length+' 条</span><span class="muted">· 共 '+knowledgeDocumentRuntime.records.length+' 条正式知识</span></div>')};
const fetchPublishedKnowledgePage=async(page,records=[])=>{const response=await fetch('/api/v1/institution/knowledge-documents?page='+page,{cache:'no-store',credentials:'same-origin'});const payload=await response.json().catch(()=>null);if(!response.ok||!Array.isArray(payload?.records))throw new Error('正式知识读取失败');const nextRecords=records.concat(payload.records);if(payload?.pageInfo?.hasMore&&page<100)return fetchPublishedKnowledgePage(page+1,nextRecords);return nextRecords};
const hydratePublishedKnowledge=()=>{if(state.route!=='/knowledge')return;if(knowledgeDocumentRuntime.loaded||knowledgeDocumentRuntime.loading){renderPublishedKnowledge();return}knowledgeDocumentRuntime={...knowledgeDocumentRuntime,loading:true,error:''};renderPublishedKnowledge();fetchPublishedKnowledgePage(1).then(records=>{knowledgeDocumentRuntime={loading:false,loaded:true,error:'',records};renderPublishedKnowledge()}).catch(error=>{knowledgeDocumentRuntime={loading:false,loaded:true,error:error instanceof Error?error.message:'正式知识读取失败',records:[]};renderPublishedKnowledge()})};
