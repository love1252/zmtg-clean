...
const APPROVED_PRESENTATION_REFINEMENT_SCRIPT = [
  ...
  '(()=>{',
  "const previewAccount=Object.freeze({name:'admin',role:'机构管理员',institution:'上海美颜'});",
  ...
  'const accountSecurityModal=()=>{closeAll();openModal(\'账号安全\',`<div class="detail"><div class="drow"><span class="dkey">登录账号</span><span class="dvalue">${previewAccount.name}</span></div><div class="drow"><span class="dkey">二次验证</span><span class="dvalue">已开启</span></div><div class="drow"><span class="dkey">当前会话</span><span class="dvalue">已通过机构端认证</span></div></div><div class="rule">密码、二次验证和会话安全由登录系统统一管理，不在机构工作台中保存凭证。</div>`,`${btn(\'关闭\',{action:\'close-overlays\'})}`)};',
  'const logoutConfirmation=()=>{closeAll();openModal(\'退出登录\',\'<p style="margin-top:0;line-height:1.75">确认退出当前 admin 账号吗？退出后需要重新登录才能进入机构工作台。</p><div class="rule">退出将清除当前正式会话与本地演示会话 Cookie，不会修改机构业务数据。</div>\',`${btn(\'取消\',{action:\'close-overlays\'})}${btn(\'确认退出\',{cls:\'danger\',action:\'preview-confirm-logout\'})}`)};',
  ...
  "if(avatar&&avatar.textContent!=='A')avatar.textContent='A';",
  ...
  "const updated=current.replace(/^(上午好|下午好|晚上好)，.+$/,'$1，admin');",
...
let customerListRuntime={page:1,pageSize:20,records:[],pageInfo:null,loading:false,error:'',requestKey:'',requestToken:0,expanded:false,lifecycle:null,priority:null,segmentLabel:''};
...
const clarifyCustomerListDataSource=()=>{if(state.route!=='/customers/list')return;const page=document.querySelector('#page');const status=page?.querySelector('.status-banner');if(status&&!status.dataset.customerRuntimeSource){status.dataset.customerRuntimeSource='true';status.innerHTML=ico('database')+'<span>客户表格、分页与分群人数来自当前本地开发数据库的正式 Customer Reader；未读取手机号等敏感字段。</span>'}const scope=status?.nextElementSibling;if(scope?.classList.contains('card')&&!scope.dataset.customerRuntimeScope){scope.dataset.customerRuntimeScope='true';scope.innerHTML=ico('shield')+'<span>当前仅显示登录账号有权访问的 tenantId + institutionId 客户主档；来源、随访、微信身份和负责人未接入正式投影时会明确标注，不采用原型 Demo 值。</span><span class="spacer"></span>'+tag('范围已校验','green')}};
...
const showImportWarnings=()=>openDrawer('导入警告明细','<div class="list-row"><div class="grow"><div class="list-title">缺少非必填字段</div><div class="list-meta">28 条 · 可继续导入</div></div>'+tag('警告','orange')+'</div><div class="list-row"><div class="grow"><div class="list-title">疑似重复客户</div><div class="list-meta">15 条 · 需要在重复处理步骤人工确认</div></div>'+tag('待处理','orange')+'</div>'+localOnly,btn('关闭',{action:'close-overlays'}));
...
if(state.importStep===5)document.querySelectorAll('#modal button').forEach(button=>{const label=textOf(button);if(label==='合并到现有客户')button.dataset.action='preview-import-merge';if(label==='作为新客户')setAction(button,'preview-import-new');if(label==='暂不处理')setAction(button,'preview-import-defer')});
...
if(action==='preview-import-merge'||action==='preview-import-new'||action==='preview-import-defer'){state.importStep=6;importWizard();toast(action==='preview-import-merge'?'已选择合并到现有客户':action==='preview-import-new'?'已选择作为新客户':'已标记为暂不处理');return true}
...
export function prepareApprovedPrototypeHtml(html: string) {
  ...
  return styledHtml.includes('</body>')
    ? styledHtml.replace(
        '</body>',
        `${APPROVED_PRESENTATION_REFINEMENT_SCRIPT}${APPROVED_INTERACTION_COMPLETION_STYLE}${APPROVED_INTERACTION_COMPLETION_SCRIPT}</body>`,
      )
    : styledHtml;
}
