...
import type {
  WeComCustomerMappingReviewMappingSnapshot,
...
const fixtureMetadata: Record<string, FixtureMetadata> = {
  ...
  'tenant-mock-002': {
    ...
    candidateReference: 'candidate-tenant-mock-002',
    dataMode: 'mock',
    sourceKind: 'controlled_mock_fixture',
    authorizationStatus: 'authorized',
  ...
  'tenant-mock-003': {
    ...
    candidateReference: 'candidate-tenant-mock-003',
    dataMode: 'mock',
    sourceKind: 'controlled_mock_fixture',
    authorizationStatus: 'authorized',
  ...
  'tenant-demo-001': {
    ...
    candidateReference: 'candidate-tenant-demo-001',
    dataMode: 'demo',
  ...
  'tenant-mock-004': {
    ...
    candidateReference: 'candidate-tenant-mock-004',
    dataMode: 'mock',
    sourceKind: 'controlled_mock_fixture',
    authorizationStatus: 'authorized',
    providerStatus: 'mock_only',
  ...
  'tenant-mock-005': {
    ...
    candidateReference: 'candidate-tenant-mock-005',
    dataMode: 'mock',
    sourceKind: 'controlled_mock_fixture',
    authorizationStatus: 'authorized',
    providerStatus: 'mock_only',
...
function buildRawView(
  ...
  reviewSnapshot: WeComCustomerMappingReviewMappingSnapshot,
  ...
  if (
    result.nextState.mappings.length !== 1
    || !mapping
    || !target
  ) {
    return blockedRawView(
      tenantId,
      metadata,
      'response_contract_invalid',
    );
  ...
  const reviewDisplay = getWeComCustomerMappingReviewDisplay(
    reviewSnapshot.mappingReviewStatus,
  );
  if (!reviewDisplay) {
    return blockedRawView(
      tenantId,
  ...
  return {
    ...
    mockDemo: true,
    readonly: true,
    mappingId: reviewSnapshot.mappingId,
    mappingVersion: reviewSnapshot.mappingVersion,
    mappingReviewStatus: reviewSnapshot.mappingReviewStatus,
    authorizationStatus: metadata.authorizationStatus,
    providerStatus: metadata.providerStatus,
    candidates: [{
...
export function readWeComCustomerMappingCandidateReference(
  tenantId: string,
): string | null {
  return fixtureMetadata[tenantId]?.candidateReference ?? null;
}

export function readWeComCustomerMappingCandidates(
  tenantId: string,
  reviewSnapshot: WeComCustomerMappingReviewMappingSnapshot | null,
  ...
  if (!reviewSnapshot) {
    return parseWeComCustomerMappingCandidatesResponse(
      blockedRawView(tenantId, metadata, 'tenant_fixture_unavailable'),
      tenantId,
    )!;
  }
  const raw = buildRawView(tenantId, metadata, result, reviewSnapshot);
