...
const downstream = vi.hoisted(() => ({
  ...
  isExecutionReady: vi.fn(() => {
    throw new Error('config readiness must not be evaluated');
  }),
  readConfig: vi.fn(() => {
...
vi.mock('@/modules/institution/domain/wecom-customer-contact-readonly-proof', () => ({
  evaluateWeComCustomerContactReadonlyProof: downstream.evaluateProof,
  isWeComCustomerContactReadonlyProofExecutionReady: downstream.isExecutionReady,
  summarizeWeComCustomerContactReadonlyProofConfig: downstream.summarizeConfig,
  weComCustomerContactReadonlyProofAction: 'read_single_external_contact_once',
  weComCustomerContactReadonlyProofConfirmation:
    'CONFIRM_READ_SINGLE_EXTERNAL_CONTACT_ONCE',
}));

vi.mock('@/modules/institution/server/wecom-customer-contact-readonly-proof-runtime', () => ({
  createWeComCustomerContactReadonlyProofClient: downstream.createClient,
  readWeComCustomerContactReadonlyProofConfig: downstream.readConfig,
}));
...
describe('企业微信客户联系只读证明 capability-off API', () => {
  it.each([
    [
      '普通 GET',
      GET,
      new Request('http://localhost/api/institution/wecom-customer-contact-readonly-proof'),
    ],
    [
      '带伪造 query、cookie 与机构头的 GET',
      GET,
      new Request(
        'http://localhost/api/institution/wecom-customer-contact-readonly-proof?externalUserId=SENSITIVE_CONTACT&tenantId=SENSITIVE_TENANT',
        {
          headers: {
    ...
    [
      ...
      POST,
      new Request(
        'http://localhost/api/institution/wecom-customer-contact-readonly-proof?payload=SENSITIVE_QUERY',
        {
          method: 'POST',
          headers: {
            authorization: 'Bearer SENSITIVE_TOKEN',
          ...
          body: '{"externalUserId":"SENSITIVE_BODY"',
        },
      ),
    ],
    ...
    expect(serialized).not.toMatch(
      /SENSITIVE_|tenantId|institutionId|externalUserId|token|secret|payload/iu,
    );
  });
