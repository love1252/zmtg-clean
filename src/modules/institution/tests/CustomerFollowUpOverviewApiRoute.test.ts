...
vi.mock('@/modules/institution/server/followup-customer-timeline-service', () => ({
  getCustomerFollowUpOverview: routeMocks.getCustomerFollowUpOverview,
}));
...
describe('机构端客户随访概览 capability gate', () => {
  it('普通、查询和 Cookie 请求固定返回同一低敏且不缓存的 503，不回显输入或触发下游', async () => {
    ...
    for (const request of [
      new Request('http://localhost/api/institution/customers/customer_safe_001/followup-overview'),
      new Request(
      ...
      const response = await customerFollowUpOverviewGet(request, {
    ...
    expect(routeMocks.getCustomerFollowUpOverview).not.toHaveBeenCalled();
  ...
  it('固定返回低敏 503，且 hostile Request 和 context 不触发任何读取或服务副作用', async () => {
    ...
    try {
      const response = await customerFollowUpOverviewGet(hostileRequest, hostileContext as never);
      const payload = await response.json();
      ...
      expect(payload).not.toHaveProperty('enrollment');
  ...
  it('消费者将 capability-disabled 响应保持为稳定的不可用错误态', async () => {
    ...
    await expect(
      getCustomerFollowUpOverview('customer_safe_001', { fetcher }),
    ).resolves.toEqual({
      ok: false,
      error: {
