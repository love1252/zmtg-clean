  ...
  () => ({
    isInstitutionRequestAuthorizationV1(value: unknown) {
      return (
        value !== null &&
        typeof value === 'object' &&
        routeMocks.genuineAuthorizations.has(value)
      );
    },
  }),
);
  ...
  () => ({
    isInstitutionSectionAllowV1(value: unknown) {
      return (
        value !== null &&
        typeof value === 'object' &&
        routeMocks.genuineSectionAllows.has(value)
      );
    },
  }),
);
...
vi.mock(
  '@/modules/security/server/institution-object-guard',
  () => ({
    isInstitutionObjectActionAllowV1(value: unknown) {
      return (
        value !== null &&
        typeof value === 'object' &&
        routeMocks.genuineObjectAllows.has(value)
      );
    },
  }),
);
...
vi.mock(
  '@/modules/institution/server/followup-customer-timeline-service',
  () => ({
    getCustomerFollowUpOverview:
      routeMocks.getCustomerFollowUpOverview,
  }),
);
...
import { GET as customerFollowUpOverviewGet } from '@/app/api/institution/customers/[customerId]/followup-overview/route';
import { getCustomerFollowUpOverview } from '@/modules/institution/client/tenant-business-client';
...
function genuineAuthorization(input: Readonly<{
  ...
  const authorization = Object.freeze({
    authorizeCurrentInstitutionSectionV1:
      input.section ??
      vi.fn(async () => genuineSectionAllow()),
    authorizeCurrentInstitutionObjectV1:
      input.object ??
      vi.fn(async () => genuineObjectAllow()),
...
describe('客户随访概览 Object Guard 最小接线', () => {
  it('Section 拒绝前不读取 Request 或 context', async () => {
    ...
    const hostileContext = new Proxy({}, {
      ...
      ownKeys() {
        contextTraps.ownKeys += 1;
        throw new Error('context must not be enumerated');
      },
      getOwnPropertyDescriptor() {
        contextTraps.descriptor += 1;
        throw new Error('context must not be described');
      },
    });
    ...
    routeMocks.resolveAuthorization.mockResolvedValueOnce(
      genuineAuthorization({ section }),
    );

    await expectObjectForbidden(
      await customerFollowUpOverviewGet(
        hostileRequest,
        hostileContext as never,
      ),
    );
    ...
    expect(requestTraps).toEqual({
      get: 0,
      ownKeys: 0,
      descriptor: 0,
    });
    expect(contextTraps).toEqual({
      get: 0,
      ownKeys: 0,
      descriptor: 0,
    });
  ...
  it('非法 customerId 在第二次授权前失败关闭', async () => {
    ...
    await expectObjectForbidden(
      await customerFollowUpOverviewGet(
        ...
        routeContext('customer id with spaces'),
      ),
    );
  ...
  it('授权通过后仍保留原 capability-disabled 503 Handler', async () => {
    ...
    const request = new Request(
      'http://localhost/api/institution/customers/customer_safe_001/followup-overview?tenantId=other-tenant&include=private',
      { headers: { cookie: 'session=secret-cookie' } },
    );
    const response = await customerFollowUpOverviewGet(
      request,
      routeContext(),
    );
  ...
  it('消费者仍将 503 保持为稳定不可用错误态', async () => {
    ...
    await expect(
      ...
      error: {
        ...
        message: '客户随访概览能力暂未启用',
        status: 503,
      },
    });
    expect(fetcher).toHaveBeenCalledWith(
      '/api/institution/customers/customer_safe_001/followup-overview',
      { cache: 'no-store' },
    );
  });
});
