...
export function InstitutionV11PageHeader({
  ...
  return (
    <header>
      <nav aria-label="面包屑" className="sr-only">
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          ...
          <h1 className="text-[24px] font-bold leading-8 tracking-tight text-slate-900">{title}</h1>
          <p className="mt-1 max-w-4xl text-[13px] leading-5 text-slate-500">{description}</p>
        </div>
...
export function InstitutionV11Button({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'inline-flex h-9 items-center justify-center gap-2 rounded-lg border px-3 text-sm font-semibold shadow-sm transition focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/40 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-400 disabled:shadow-none',
        tone === 'primary'
...
export function InstitutionV11CapabilityBanner({
  ...
  return (
    <section
      aria-label="能力状态"
      className="flex min-h-11 items-center justify-between gap-3 rounded-xl border border-amber-200 bg-amber-50/75 px-3.5 py-2"
    >
      <div className="flex min-w-0 items-center gap-2.5">
        <span className="grid h-6 w-6 shrink-0 place-items-center text-amber-700">
          <AlertTriangle aria-hidden="true" className="h-4 w-4" />
        </span>
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h2 className="text-[13px] font-medium text-amber-900">{title}</h2>
            <InstitutionV11StatusPill state={state} />
          </div>
...
export function InstitutionV11Tabs({
  ...
  return (
    <div
      ...
      className="flex min-w-max items-center gap-1 border-b border-slate-200 px-2"
    >
      {items.map((item) => {
        ...
        return (
          <button
            ...
            className={cn(
              'relative h-11 px-3.5 text-sm font-medium outline-none transition focus-visible:ring-2 focus-visible:ring-blue-500/35 disabled:cursor-not-allowed disabled:text-slate-300',
...
export function InstitutionV11Surface({
  ...
  return (
    <section className={cn('rounded-xl border border-slate-200 bg-white shadow-sm shadow-slate-200/40', className)}>
...
export function InstitutionV11UnavailableValue({
  ...
  return (
    <article className="rounded-xl border border-slate-200 bg-white px-4 py-4 shadow-sm shadow-slate-200/40">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-xl font-bold tracking-tight text-slate-400">—</p>
        </div>
      </div>
      <p className="mt-1 truncate pl-[52px] text-[10px] text-slate-400">来源：{source}</p>
    </article>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-[70] flex justify-end bg-slate-950/35 backdrop-blur-[1px]" onMouseDown={onClose}>
      <section
        ...
        className="flex h-full w-full max-w-[660px] flex-col border-l border-slate-200 bg-white shadow-2xl shadow-slate-950/25"
...
export function InstitutionV11Modal({
  open,
  title,
  onClose,
  children,
  ...
  return (
    <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-900/35 p-5" onMouseDown={onClose}>
      <section ref={panelRef} role="dialog" aria-modal="true" aria-labelledby={titleId} className="w-full max-w-[820px] overflow-hidden rounded-2xl bg-white shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
        <header className="flex h-16 items-center justify-between border-b border-slate-200 px-5"><h2 id={titleId} className="text-xl font-bold text-slate-900">{title}</h2><button ref={closeRef} type="button" aria-label={`关闭${title}`} onClick={onClose} className="grid h-9 w-9 place-items-center rounded-lg text-slate-500 hover:bg-slate-100"><X className="h-4 w-4" /></button></header>
