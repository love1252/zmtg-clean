...
import { InstitutionV11UnavailableValue } from '@/modules/institution-v11/components/InstitutionV11Ui';
...
export function InstitutionWorkbenchShell({
  ...
  return (
    ...
    >
      <div id="institution-workbench-heading" className="flex items-start justify-between gap-4">
        {hasVisibleProjection ? <span className="sr-only">聚合已授权的预约、随访与会话信息，帮助团队优先处理需要关注的事项。</span> : null}
        <div>
          <h1 className="sr-only">工作台</h1>
          <h2 className="text-[26px] font-bold leading-9 tracking-tight text-slate-900">上午好，机构成员</h2>
          <p className="mt-1 text-[13px] text-slate-500">以下是今日重点业务动态；工作台负责行动和异常，不替代完整业务页面。</p>
          {!hasVisibleProjection ? <p className="sr-only">当前仅展示安全阻断状态；业务数据和业务入口保持隐藏。</p> : null}
        </div>
      ...
      <section aria-labelledby="workbench-kpi-heading">
        <h2 id="workbench-kpi-heading" className="sr-only">今日概览</h2>
      ...
      {hasVisibleProjection ? null : (
        <>
          <div className="grid gap-4 xl:grid-cols-[1.15fr_1fr]">
            <section aria-labelledby="workbench-unavailable-heading" className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
              <header className="flex h-[52px] items-center justify-between border-b border-slate-200 px-4"><h2 id="workbench-unavailable-heading" className="text-[15px] font-semibold text-slate-900">数据服务/能力尚未安全开放</h2><span className="rounded-lg bg-amber-50 px-2 py-1 text-[11px] font-semibold text-amber-700">能力未开放</span></header>
              <div className="flex min-h-[220px] items-start gap-3 p-4">
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-amber-50 text-amber-700"><LockKeyhole aria-hidden="true" className="h-5 w-5" /></span>
                <div className="min-w-0 text-xs leading-5 text-slate-500">
                  <h3 className="text-sm font-semibold text-slate-800">工作台行动数据暂未开放</h3>
                  <p className="mt-2">待确认预约、改约申请、逾期随访、今日到期随访和行动队列当前保持隐藏。</p>
                  <p className="mt-2">只有可验证来源、机构隔离、服务端成员与对象权限完成后，才会显示低敏投影。</p>
                  <p className="mt-3 flex items-center gap-2 rounded-lg bg-slate-50 px-3 py-2"><CircleAlert aria-hidden="true" className="h-4 w-4 shrink-0" />这不是“零数据”状态，因此未知值不会显示为 0。</p>
                </div>
              </div>
            </section>
            <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
              <header className="flex h-[52px] items-center justify-between border-b border-slate-200 px-4"><h2 className="text-[15px] font-semibold text-slate-900">今日安排</h2><span className="text-xs font-semibold text-slate-400">日程未开放</span></header>
              <div className="divide-y divide-slate-100 px-4">{['09:30', '10:30', '14:00', '15:30'].map((time) => <div key={time} className="flex h-[52px] items-center gap-3"><span className="w-12 text-xs font-semibold text-slate-700">{time}</span><span className="grid h-8 w-8 place-items-center rounded-full bg-slate-100 text-xs text-slate-400">—</span><span className="text-xs text-slate-400">安排数据不可用</span></div>)}</div>
            </section>
          </div>
          <div className="grid gap-4 xl:grid-cols-[0.9fr_0.9fr_1.35fr]">
            {['风险与机会', '客户动态', '近期业务趋势'].map((title) => <section key={title} className="min-h-[230px] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><header className="flex h-[52px] items-center border-b border-slate-200 px-4"><h2 className="text-[15px] font-semibold text-slate-900">{title}</h2></header><div className="grid min-h-[176px] place-items-center px-4 text-xs text-slate-400">正式聚合未开放</div></section>)}
          </div>
        </>
      )}
