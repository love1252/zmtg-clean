...
const commercialLifecycle = [
  {
    ...
    icon: WalletCards,
    status: '词汇预留',
    statusTone: 'border-slate-300/20 bg-slate-300/[0.06] text-slate-300',
    items: [
    ...
    <section className="space-y-5" aria-labelledby="commercial-boundary-heading">
      <div className={cn(sectionShell, 'overflow-hidden')}>
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            ...
            <h2 id="commercial-boundary-heading" className="mt-4 text-2xl font-semibold tracking-normal text-white sm:text-3xl">
              商业化边界
            </h2>
            ...
            <article key={phase.phase} className={sectionShell}>
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between gap-3">
                    <h3 className="text-lg font-semibold tracking-normal text-white">{phase.phase}</h3>
                    <span className={cn('rounded-full border px-2.5 py-0.5 text-xs font-semibold', phase.statusTone)}>
                      {phase.status}
                    </span>
                  </div>
                  <p className="mt-1 text-sm text-slate-400">当前仅为治理词汇预留，不启用真实操作。</p>
                </div>
              </div>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                {phase.items.map((item) => (
                  <div key={item.label} className="rounded-2xl border border-white/10 bg-[#071322]/72 p-4">
                    <h4 className="text-sm font-semibold tracking-normal text-white">{item.label}</h4>
      ...
      <article className={sectionShell}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold tracking-normal text-white">商业化健康信号</h3>
            <p className="mt-1 text-sm text-slate-400">商业化健康审计维度为运营参考，不关联真实计费系统。</p>
          </div>
        </div>

        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          {auditSignals.map((signal) => (
            <div key={signal.key} className="rounded-2xl border border-white/10 bg-[#071322]/72 p-4">
              <h4 className="text-sm font-semibold tracking-normal text-white">{signal.label}</h4>
              <p className="mt-2 text-sm leading-5 text-slate-400">{signal.desc}</p>
              <div className="mt-3 rounded-full border border-slate-400/20 bg-slate-400/[0.06] px-2.5 py-0.5 text-xs font-semibold text-slate-400 inline-block">
                {signal.key}
              </div>
            </div>
          ))}
