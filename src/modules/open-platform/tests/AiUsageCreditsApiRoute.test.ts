...
describe('平台端 AI usage credits API', () => {
  it('平台端可获取汇总和明细，并传递过滤条件', async () => {
    ...
    expect((body.aggregations as Record<string, unknown[]>).byDateProvider[0]).toMatchObject({
      date: '2026-06-30',
      provider: 'deepseek',
      providerDisplayName: 'DeepSeek',
      totalCalls: 2,
    ...
    expect((body.aggregations as Record<string, unknown[]>).byDateProviderModel[0]).toMatchObject({
      date: '2026-06-30',
      provider: 'deepseek',
      providerDisplayName: 'DeepSeek',
      model: 'deepseek-v4-flash',
