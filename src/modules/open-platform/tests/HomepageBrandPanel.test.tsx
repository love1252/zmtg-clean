...
describe('首页与品牌面板', () => {
  ...
  it('品牌概览的页面基础信息包含 SEO、页脚和二维码字段', async () => {
    render(<HomepageBrandPanel />);

    await screen.findByRole('heading', { name: '首页与品牌' });
    const basicInfo = screen.getByRole('article', { name: '页面基础信息' });
    expect(within(basicInfo).getByLabelText('SEO标题')).toBeInTheDocument();
    expect(within(basicInfo).getByLabelText('SEO关键词')).toBeInTheDocument();
    expect(within(basicInfo).getByLabelText('SEO描述')).toBeInTheDocument();
    expect(within(basicInfo).getByLabelText('公司名称')).toBeInTheDocument();
    expect(within(basicInfo).getByLabelText('联系电话')).toBeInTheDocument();
    expect(within(basicInfo).getByLabelText('邮箱地址')).toBeInTheDocument();
    expect(within(basicInfo).getByLabelText('ICP备案号')).toBeInTheDocument();
    expect(within(basicInfo).getByLabelText('ICP备案链接')).toBeInTheDocument();
    expect(within(basicInfo).getByLabelText('公安网警备案号')).toBeInTheDocument();
    expect(within(basicInfo).getByLabelText('公安网警备案链接')).toBeInTheDocument();
    expect(within(basicInfo).getByLabelText('公众号二维码地址')).toBeInTheDocument();
    expect(within(basicInfo).getByLabelText('小程序二维码地址')).toBeInTheDocument();

    fireEvent.change(within(basicInfo).getByLabelText('公司名称'), { target: { value: '智美天工科技有限公司' } });
    expect(within(basicInfo).getByDisplayValue('智美天工科技有限公司')).toBeInTheDocument();
  });
});
