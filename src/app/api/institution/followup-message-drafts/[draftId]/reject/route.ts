...
import { rejectMessageDraft } from '@/modules/institution/server/followup-message-draft-service';
...
export async function POST(
  ...
  try {
    ...
    return await db.transaction(async (transactionDatabase) => {
      ...
      const result = await rejectMessageDraft({
        context,
        draftId,
      ...
      if (result.kind === 'updated') {
        await transactionAuditRepository.record(allowedFollowUpMessageAudit({ context, action: 'update', reason: 'message_draft_rejected', occurredAt, resourceId: result.draft.draftId }));
