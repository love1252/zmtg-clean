...
import { searchInstitutionKnowledgeVectorChunksService } from '@/modules/institution/server/institution-knowledge-vector-search-service';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await searchInstitutionKnowledgeVectorChunksService({
    ...
    return NextResponse.json(
      { code: 'service_unavailable', error: '知识库向量检索暂时不可用' },
      { status: 503 },
    );
  }
}
