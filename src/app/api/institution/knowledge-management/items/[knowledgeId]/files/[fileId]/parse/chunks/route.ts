...
export async function GET(request: Request, context: ChunksRouteContext) {
  ...
  try {
    ...
    const result = await listInstitutionKnowledgeDocumentFileChunksService({
    ...
    const resultStatus = 'status' in result && typeof result.status === 'string'
      ...
      : null;
    if (resultStatus) {
      return NextResponse.json(errorPayloadForStatus(resultStatus), {
        status: statusCodeForResult(resultStatus),
      });
    }
