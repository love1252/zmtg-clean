...
function extractAction(value: unknown): WeComOfficialSecretPrecheckAction {
  if (typeof value !== 'object' || value === null) return 'preflight';
  const action = (value as { action?: unknown }).action;
  return isPrecheckAction(action) ? action : 'preflight';
}

function forbiddenResponse(request: Request): NextResponse | null {
