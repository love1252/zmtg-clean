...
import { searchPlatformKnowledgeVectorChunksService } from '@/modules/open-platform/server/platform-knowledge-embedding-vector-search-service';
...
export async function GET(request: Request) {
  try {
    ...
    const result = await searchPlatformKnowledgeVectorChunksService({
    ...
    return NextResponse.json(
      buildReadonlyApiError('知识库向量检索暂时无法处理'),
      { status: 400 },
    );
  }
}
