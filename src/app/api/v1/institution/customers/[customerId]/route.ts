...
import {
  mutateCurrentInstitutionCustomerControlledV1,
  readCurrentInstitutionCustomerControlledV1,
...
export async function GET(request: Request, context: Context) {
  ...
  const result = await readCurrentInstitutionCustomerControlledV1(customerId);
  ...
  return NextResponse.json(
    { code: `institution_customer_${result.kind}` },
...
export async function PATCH(request: Request, context: Context) {
  ...
  const { customerId } = await context.params;
  const result = await mutateCurrentInstitutionCustomerControlledV1(
    customerId,
    body,
  );
