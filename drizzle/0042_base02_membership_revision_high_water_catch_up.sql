...
DO $migration$
DECLARE
  expected_predecessor_count CONSTANT integer := 42;
  expected_predecessor_when CONSTANT bigint := 1785614756000;
  expected_predecessor_hash CONSTANT text :=
    '85efc99f76a871cb90cf5beb3d2c6fa5ae0d70665f3821d5e42f9d30125bb591';
  ...
  pre_legacy_exact_head_count bigint;
  pre_duplicate_command_count bigint;
  pre_duplicate_revision_count bigint;
  pre_identity_collision_count bigint;
  ...
  post_legacy_exact_head_count bigint;
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_POSTGRES_VERSION_DRIFT';
  END IF;
  ...
  THEN
    RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_IDENTITY_FUNCTION_DRIFT';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_IDENTITY_VECTOR_DRIFT';
  END IF;

  IF pg_catalog.to_regclass('drizzle.__drizzle_migrations') IS NULL THEN
    RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_JOURNAL_MISSING';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_JOURNAL_DRIFT';
  END IF;
  ...
  THEN
    RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_REQUIRED_RELATION_MISSING';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_ENUM_DRIFT';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_CURRENT_CATALOG_DRIFT';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_TRANSITION_CATALOG_DRIFT';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_USER_FK_DRIFT';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_SCOPE_FK_DRIFT';
  END IF;
  ...
  JOIN public.tenant_membership_transitions head_transition
   ...
   AND head_transition.occurred_at IS NOT DISTINCT FROM
     current_member.current_provenance_occurred_at
   AND head_transition.recorded_at = current_member.current_provenance_recorded_at
   AND (
     current_member.current_provenance_source <> 'legacy_calibration'
     OR head_transition.transition_type = 'legacy_calibration'
   )
  WHERE current_member.revision IS NOT NULL
    AND current_member.lifecycle_status IS NOT NULL
    AND current_member.current_provenance_source IS NOT NULL
    AND current_member.current_provenance_reason_code IS NOT NULL
    AND current_member.current_provenance_command_id IS NOT NULL
    AND current_member.current_provenance_recorded_at IS NOT NULL;

  SELECT count(*) INTO pre_legacy_exact_head_count
  FROM public.tenant_members current_member
  JOIN public.tenant_membership_transitions head_transition
    ON head_transition.tenant_id = current_member.tenant_id
   AND head_transition.membership_id = current_member.id
   AND head_transition.command_id = current_member.current_provenance_command_id
   AND head_transition.to_revision = current_member.revision
  WHERE current_member.revision = 1
    AND current_member.lifecycle_status = 'active'
    AND current_member.current_provenance_source = 'legacy_calibration'
    AND current_member.current_provenance_actor_id IS NULL
    AND current_member.current_provenance_reason_code = 'legacy_unknown'
    AND current_member.current_provenance_command_id = (
      'mcal1_' || pg_catalog.encode(
        pg_catalog.sha256(
          pg_catalog.convert_to(command_domain, 'UTF8')
          || pg_catalog.decode('00', 'hex')
          || pg_catalog.convert_to(current_member.tenant_id, 'UTF8')
          || pg_catalog.decode('00', 'hex')
          || pg_catalog.convert_to(current_member.id, 'UTF8')
        ),
        'hex'
      )
    )
    AND current_member.current_provenance_occurred_at IS NULL
    AND current_member.current_provenance_recorded_at IS NOT NULL
    AND current_member.revoked_at IS NULL
    AND current_member.deleted_at IS NULL
    AND head_transition.id = (
      'mtcl1_' || pg_catalog.encode(
        pg_catalog.sha256(
          pg_catalog.convert_to(transition_domain, 'UTF8')
          || pg_catalog.decode('00', 'hex')
    ...
    AND head_transition.transition_type = 'legacy_calibration'
    AND head_transition.source = 'legacy_calibration'
    AND head_transition.actor_id IS NULL
    AND head_transition.reason_code = 'legacy_unknown'
    AND head_transition.from_revision IS NULL
    AND head_transition.to_revision = 1
    AND head_transition.from_lifecycle_status IS NULL
    AND head_transition.to_lifecycle_status = 'active'
    AND head_transition.from_role IS NULL
    AND head_transition.to_role = current_member.role
    AND head_transition.occurred_at IS NULL
    AND head_transition.recorded_at = current_member.current_provenance_recorded_at;

  SELECT count(*) INTO pre_duplicate_command_count
  FROM (
    SELECT transition_row.tenant_id, transition_row.command_id
    FROM public.tenant_membership_transitions transition_row
    GROUP BY transition_row.tenant_id, transition_row.command_id
    HAVING count(*) > 1
  ) duplicate_command;
  ...
  FROM (
    SELECT
      ...
      transition_row.membership_id,
      transition_row.to_revision
    FROM public.tenant_membership_transitions transition_row
    GROUP BY
      transition_row.tenant_id,
      transition_row.membership_id,
  ...
  FROM public.tenant_members residual_member
  WHERE pg_catalog.num_nonnulls(
    residual_member.revision,
    residual_member.lifecycle_status,
    residual_member.current_provenance_source,
    residual_member.current_provenance_actor_id,
    residual_member.current_provenance_reason_code,
    residual_member.current_provenance_command_id,
    residual_member.current_provenance_occurred_at,
    residual_member.current_provenance_recorded_at,
    residual_member.revoked_at,
    residual_member.deleted_at
  ) = 0
    AND (
      EXISTS (
        SELECT 1
        FROM public.tenant_members existing_member
        WHERE existing_member.current_provenance_command_id = (
          'mcal1_' || pg_catalog.encode(
            pg_catalog.sha256(
              pg_catalog.convert_to(command_domain, 'UTF8')
              || pg_catalog.decode('00', 'hex')
              || pg_catalog.convert_to(residual_member.tenant_id, 'UTF8')
              || pg_catalog.decode('00', 'hex')
              || pg_catalog.convert_to(residual_member.id, 'UTF8')
            ),
            'hex'
          )
        )
      ...
      OR EXISTS (
        ...
        WHERE existing_transition.command_id = (
          'mcal1_' || pg_catalog.encode(
            pg_catalog.sha256(
              pg_catalog.convert_to(command_domain, 'UTF8')
              || pg_catalog.decode('00', 'hex')
          ...
          OR (
            existing_transition.tenant_id = residual_member.tenant_id
            AND existing_transition.membership_id = residual_member.id
  ...
  IF pre_membership_count < 1
    OR pre_transition_count < pre_complete_count
    OR pre_membership_count <> pre_all_null_count + pre_complete_count
    OR pre_exact_current_head_count <> pre_complete_count
    OR pre_legacy_exact_head_count <> 1
    OR pre_duplicate_command_count <> 0
    OR pre_duplicate_revision_count <> 0
    OR pre_identity_collision_count <> 0
  ...
  THEN
    RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_DATA_BASELINE_DRIFT';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_CANDIDATE_PARENT_MISSING';
  END IF;

  IF pre_all_null_count = 0 THEN
  ...
  ELSE
    ...
    IF high_water_created_at IS NULL OR high_water_id IS NULL THEN
      RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_HIGH_WATER_MISSING';
    END IF;
    ...
    IF planned_count <> pre_all_null_count THEN
      RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_PLANNED_COUNT_DRIFT';
    END IF;
  ...
  LOOP
    ...
    THEN
      ...
      RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_IDENTITY_CONFLICT';
    END IF;
    ...
    THEN
      ...
      RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_CURRENT_UPDATE_DRIFT';
    END IF;
    ...
    IF insert_row_count <> 1 THEN
      unexpected_count := unexpected_count + 1;
      RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_TRANSITION_INSERT_DRIFT';
    END IF;
  ...
  SELECT count(*) INTO post_legacy_exact_head_count
  FROM public.tenant_members current_member
  JOIN public.tenant_membership_transitions head_transition
    ON head_transition.tenant_id = current_member.tenant_id
   AND head_transition.membership_id = current_member.id
  ...
  IF planned_count <> created_count + reused_count
    ...
    OR post_all_null_count <> 0
    OR post_partial_count <> 0
    OR post_complete_count <> pre_complete_count + created_count
    OR post_exact_current_head_count <> pre_exact_current_head_count + created_count
    OR post_exact_current_head_count <> post_complete_count
    OR post_legacy_exact_head_count <> pre_legacy_exact_head_count + created_count
  ...
  THEN
    RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_COUNT_POSTCHECK_FAILED';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_CURRENT_EVIDENCE_POSTCHECK_FAILED';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION USING MESSAGE = 'BASE02_MEMBERSHIP_M5_JOURNAL_POSTCHECK_FAILED';
  END IF;
END
$migration$;
