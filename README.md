...
- Phase 23 HIS 连接配置凭证 compensation worker claim / lock / stale recovery Plan Mode 已完成规划：只读盘点确认 operation repository 与 job queue repository 已具备 worker 编排所需最小持久化边界，后续 worker runtime 需单独实现 claim、job running、operation running、stale locked job recovery、stale running operation recovery、retry / dead letter / manual review 分流和 audit integration；当前 PR 仅文档，不修改 `src/**`、不修改 `drizzle/**`、不新增 schema / migration、不实现 worker runtime、不调用 provider、不接 service、不处理真实凭证、不做测试连接、不接真实 HIS adapter
- 开放平台基础治理基线
