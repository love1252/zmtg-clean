...
- Phase 23 HIS 连接配置凭证 compensation worker claim / lock / stale recovery runtime 最小边界已完成：新增 `createHisConnectionCredentialCompensationWorker` 编排 factory，基于 operation repository 与 job queue repository 完成 due job claim、job running、operation running、expired lock 保守 recovery 和 stale running operation manual review 分流；本阶段不新增 runner / scheduler，不接 service，不调用 provider，不写 compensation audit，不处理真实凭证，不做测试连接，不接真实 HIS adapter，不新增 API route，不新增 schema / migration
- Phase 23 HIS 连接配置凭证 compensation worker test-only provider / no-op execution Plan Mode 已完成：只读盘点确认 worker、operation repository 与 job queue repository 已足够支撑后续 provider execution 的 success / failure / manual review 收口规划；本阶段仅新增中文 Plan / Spec 文档并同步 README、roadmap 和 devlog，不修改 `src/**`、不修改 `drizzle/**`，不实现 provider runtime，不调用真实 provider，不处理真实凭证，不做测试连接，不接真实 HIS adapter，不写 compensation audit，不新增 runner / scheduler
- Phase 23 HIS 连接配置凭证 compensation worker no-op provider execution 最小边界已完成：worker 新增 test-only / no-op `providerExecutor` 注入点和 `executeClaimedCredentialCompensationJob`，只接收安全上下文，支持 `success`、`retryable_failure`、`unsafe_unknown`、`provider_unavailable`、`timeout` 的稳定 completion 收口；本阶段不新增 schema / migration，不修改 `drizzle/**`，不接真实 provider，不处理真实凭证，不做测试连接，不接真实 HIS adapter，不写 compensation audit，不实现 retry / requeue / backoff，不新增 runner / scheduler
- Phase 23 HIS 连接配置凭证 compensation retry / requeue / backoff runtime Plan Mode 已完成：只读盘点确认 worker 已具备 retry / requeue 规划入口，job queue repository 已支持 `requeueCredentialCompensationJob`、`retryCount`、`maxRetryCount`、`nextAttemptAt` 与 `claimId + claimVersion` 写回保护，operation repository 已支持 running / failed / retry count 对齐；本阶段仅新增中文 Plan / Spec 文档并同步 README、roadmap 和 devlog，不修改 `src/**`、不修改 `drizzle/**`，不新增 schema / migration，不实现 retry / requeue / backoff、dead letter、manual review 或 compensation audit，不接真实 provider，不处理真实凭证，不做测试连接，不接真实 HIS adapter，不新增 runner / scheduler
- Phase 23 HIS 连接配置凭证 compensation retry policy 纯决策 helper 最小边界已完成：新增 `decideHisConnectionCredentialCompensationRetry`，固定 provider result、decision 与 reason 白名单，覆盖 `retryable_failure` / `provider_unavailable` 未达上限 requeue、达到上限 dead letter、`timeout` / `unsafe_unknown` manual review、`validation_failed` / `repository_error` no retry，以及 fixed backoff、jitter 和 `nextAttemptAt` 计算；本阶段不修改 worker runtime，不实现 requeue runtime，不实现 dead letter / manual review runtime，不新增 schema / migration，不修改 `drizzle/**`，不接真实 provider，不处理真实凭证，不做测试连接，不接真实 HIS adapter，不写 compensation audit，不新增 runner / scheduler；下一步进入 worker retry / requeue runtime 最小实现或 operation retry count 对齐 Plan Mode
- Phase 23 HIS 连接配置凭证 compensation worker retry / requeue runtime 最小边界已完成：worker 在 `retryable_failure` 与 `provider_unavailable` 的 failed completion 后复用 `decideHisConnectionCredentialCompensationRetry`，未达上限时使用 helper 返回的 `nextAttemptAt` 调用 `requeueCredentialCompensationJob`，写回携带 `tenantId + connectionId + operationId + claimId + claimVersion`；本阶段不直接修改 retryCount，不接 operation retry count 对齐，不实现 dead letter / manual review runtime，不写 compensation audit，不接真实 provider，不处理真实凭证，不做测试连接，不接真实 HIS adapter，不新增 runner / scheduler，不新增 schema / migration
- Phase 23 HIS 连接配置凭证 compensation operation retry count 对齐规划已完成：只读盘点确认 job `retryCount` 是调度事实并由 job queue requeue 递增，operation `retryCount` 只做业务状态展示与补偿事实对齐，不参与 retry policy 或 job 调度；推荐后续运行时在 requeue 成功后递增 operation retry count，递增失败不回滚 job requeue，允许短暂不一致并由后续恢复流程、人工复核和可观测性兜底；本阶段仅新增中文规划文档并同步 README、roadmap 和 devlog，不修改 `src/**` 或 `drizzle/**`，不新增 schema / migration，不实现运行时，不接 audit / service / route / runner / scheduler，不接真实 provider，不处理真实凭证，不做测试连接，不接真实 HIS adapter
- Phase 23 HIS 连接配置凭证 compensation operation retry count 对齐 runtime 最小边界已完成：worker 仅在 `requeueCredentialCompensationJob` 成功后调用 `incrementCredentialCompensationOperationRetryCount`，调用输入绑定 `tenantId + connectionId + operationId`；operation retry count 失败不回滚 job requeue，并收敛为现有稳定 worker result；本阶段不新增 schema / migration，不修改 job queue repository 或 operation repository，不实现 dead letter / manual review runtime，不写 audit，不接 service / route / runner / scheduler，不接真实 provider，不处理真实凭证，不做测试连接，不接真实 HIS adapter
- Phase 23 HIS 连接配置凭证 compensation retry exhausted dead letter runtime Plan Mode 已完成：只读盘点确认 retry policy helper 已在 failed job 且 `retryCount >= maxRetryCount` 时返回 `dead_letter` / `retry_exhausted`，job queue repository 已有 `markCredentialCompensationJobDeadLettered` 和 `retry_exhausted` reason；本阶段仅新增中文 Plan 文档并同步 README、roadmap 和 devlog，不修改 `src/**` 或 `drizzle/**`，不新增 schema / migration，不修改 job queue repository 或 operation repository，不实现 dead letter / manual review runtime，不接 audit / service / route / runner / scheduler，不接真实 provider，不处理真实凭证，不做测试连接，不接真实 HIS adapter
- Phase 23 HIS 连接配置凭证 compensation retry exhausted dead letter runtime 最小边界已完成：worker 在 `retryable_failure` / `provider_unavailable` 的 job failed + operation failed 成功后，若 retry policy 返回 `dead_letter` / `retry_exhausted`，调用 `markCredentialCompensationJobDeadLettered` 并携带 `tenantId + connectionId + operationId + claimId + claimVersion + now + deadLetterReason`；dead letter 失败映射为现有稳定 worker result，不回滚 job failed / operation failed，不 requeue，不递增 operation retry count，不推进 manual review；本阶段不新增 schema / migration，不修改 job queue repository 或 operation repository，不写 audit，不接 service / route / runner / scheduler，不接真实 provider，不处理真实凭证，不做测试连接，不接真实 HIS adapter
- Phase 23 HIS 连接配置凭证 compensation retry exhausted manual review Plan Mode 已完成：只读盘点确认 job `dead_lettered` 后不应再调用 job manual review，现有 operation manual review repository 仅支持 `compensation_running` 到 `manual_review_required`，不能直接复用来完成 `compensation_failed` 到 `manual_review_required`；本阶段仅新增中文 Plan 文档并同步 README、roadmap 和 devlog，不修改 `src/**` 或 `drizzle/**`，不新增 schema / migration，不实现 manual review runtime，不接 audit / service / route / runner / scheduler，不接真实 provider，不处理真实凭证，不做测试连接，不接真实 HIS adapter；后续如需 dead letter 成功后推进 operation manual review，必须先单独规划 operation repository 状态转换边界
